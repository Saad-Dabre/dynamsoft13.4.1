...
(function(a) {
	...
	Dynamsoft.WebTwainEnv = {
		...
		ProductKey: "",
		Trial: true,
		ResourcesPath: "https://saad-dabre.github.io/dynamsoft13.4.1/Resources",
		Debug: false,
	...
	j = a.html5.Funs = {
		...
		init: function(o, z) {
			var l, m = false,
				...
				u, y, n;
			y = [];
			y.push('<dialog class="dynamsoft-dwt-dialogProgress" closed="closed" style="top:30%"><h3 class="dynamsoft-finalMessage"></h3><p class="dynamsoft-status">0%</p>');
			if (a.env.bIE) {
...
(function(a) {
	...
	a.html5.STwain = function(d) {
		...
		e._width = f.width;
		e._height = f.height;
		if (g || !a.isFunction(a.UI.ImageUIView)) {
			e._bNoUI = true
		} else {
			e._bNoUI = false
		}
		...
		e.__OnMouseClick = !g && f.onMouseClick || "";
		e.__OnMouseDoubleClick = !g && f.onMouseDoubleClick || "";
		e.__OnMouseRightClick = !g && f.onMouseRightClick || "";
		e.__OnMouseMove = !g && f.onMouseMove || "";
...
(function(c) {
	...
	Dynamsoft.Lib.attachProperty = function(g) {
		...
		b(h, "AllowMultiSelect", {
			get: function() {
				return h._UIManager.GetAllowMultiSelect()
			},
			set: function(j) {
				h._UIManager.SetAllowMultiSelect(j)
			}
		});
		b(h, "BackgroundColor", {
			get: function() {
		...
		b(h, "FitWindowType", {
			get: function() {
				return h._UIManager.GetFitWindowType()
			},
			set: function(j) {
				var k = j * 1;
				return h._UIManager.SetFitWindowType(k)
			}
		});
		...
		b(h, "IfFitWindow", {
			get: function() {
				return h._UIManager.GetIfFitWindow()
			},
			set: function(j) {
				if (j) {
					return h._UIManager.SetIfFitWindow(true)
				} else {
					return h._UIManager.SetIfFitWindow(false)
				}
			}
		});
		b(h, "ImageMargin", {
			get: function() {
				return h._UIManager.GetImageMargin()
			},
			set: function(j) {
				return h._UIManager.SetImageMargin(j)
			}
		});
		...
		b(h, "SelectedImagesCount", {
			...
			set: function(k) {
				...
				h._UIManager.SetSelectedImageCount(j);
		...
		b(h, "Zoom", {
			get: function() {
				return h._UIManager.GetZoom()
			},
			set: function(j) {
				return h._UIManager.SetZoom(j)
			}
		});
		b(h, "MouseShape", {
			get: function() {
				var j = h._UIManager.GetMouseShape();
			...
			set: function(j) {
				...
				return h._UIManager.SetMouseShape(k)
			}
		});
		...
		b(h, "ShowPageNumber", {
			get: function() {
				return h._UIManager.GetShowPageNumber()
			},
			set: function(j) {
				return h._UIManager.SetShowPageNumber(j)
			}
		});
		b(h, "IfAutoScroll", {
			get: function() {
				return h._UIManager.GetIfAutoScroll()
			},
			set: function(j) {
				return h._UIManager.SetIfAutoScroll(j)
			}
		});
...
(function(a) {
	...
	function c(d) {
		var e = this;
		e.aryCurrentSelectedImageIndicesInBuffer = [];
	...
	b.UpdateAllImageInner = function(k, d, g, f, e) {
		var j = this;
		j.aryModelImageControl = k;
		j.__OnlySetCurrentIndex(d);
		j.sHowManyImages = j.aryModelImageControl.length;
		var h = (j.imagesPerRow * j.imagesPerColumn) > j.sHowManyImages ? j.sHowManyImages : j.imagesPerRow * j.imagesPerColumn;
		return true
	};
	b.__GetHowManyRows = function() {
		var d = this;
		return parseInt((d.sHowManyImages + d.imagesPerColumn - 1) / d.imagesPerColumn)
	};
	b.__GetRow = function(d) {
		var e = this;
		return parseInt(d / e.imagesPerColumn)
	};
	b.__GetFirstVisibleColumn = function() {
		var d = this;
		return parseInt(d.__GetVisibleRow(d.__GetHScrollPosionValue()))
	};
	b.__GetFirstVisibleRow = function() {
		var d = this;
		return parseInt(d.__GetVisibleRow(d.__GetVScrollPosionValue()))
	};
	b.__GetVisibleRow = function(f) {
		var e = this;
		if (e.__GetHowManyRows() == 0) {
			return 0
		}
		var d = f;
		if (e.bSingleMode) {
			return e.cCurrentIndex
		} else {
			if (e.bVerticalMode == true) {
				if (e.__GetHowManyRows() - d < e.imagesPerRow) {
					d = d - (e.imagesPerRow - (e.__GetHowManyRows() - d))
				}
			} else {
				if (e.sHowManyImages - d < e.imagesPerColumn) {
					d = d - (e.imagesPerColumn - (e.sHowManyImages - d))
				}
			}
		}
		if (d < 0) {
	...
	b.SetSelectedImageIndex = function(h, g) {
		...
		var e = -1,
			...
			j;
		for (var f = 0; f < d; f++) {
			var j = k.aryCurrentSelectedImageIndicesInBuffer[f];
			if (e == -1) {
				e = j
			} else {
				if (j < e) {
	...
	b.__Previous = function() {
		var d = this;
		d.Go(d.cCurrentIndex - 1);
		return true
	};
	b.__Next = function() {
		var d = this;
	...
	b.SwitchImage = function(f, e) {
		...
		d = j.length;
		if (f < j.length && f >= 0 && e < j.length && e >= 0) {
			h = j[f];
			h.imageIndex = e;
			g = j[e];
			g.imageIndex = f;
...
(function(a) {
	...
	b.OverlayRectangle = function(j, l, k, g, f, e, h) {
		...
		return this._UIManager.OverlayRectangle(j, l, k, g, f, e, h)
	};
	...
	b.SetSelectedImageArea = function(h, k, j, g, e) {
		...
		return l._UIManager.SetSelectedImageArea(f, k, j, g, e)
	}
})(Dynamsoft.Lib);
(function(b) {
	if (!b.product.bChromeEdition) {
	...
	d.HTTPUploadThroughPostEx = function(x, z, j, q, v, w, p) {
		var s = this,
			n = "HTTPUploadThroughPostEx",
			o = g.combineUrl(s, x, j),
			r = true,
			u = false,
			t = "",
			l = 0,
		...
		if (r) {
			switch (z) {
				case -1:
					...
					break;
				case -2:
					h = s._UIManager.GetSelectedIndexes();
					break;
				default:
					h.push(z);
					break
			...
			u = s.__GetSelectedImagesLength(v, z, r, function(A) {
				...
				var B = f.GetHttpHeadersAndClear(s);
				B["dwt-md5"] = t;
				u = s.__SaveSelectedImagesToBytes(v, 0, l, r, function(C) {
					b.needShowTwiceShowDialog = false;
					if (C && (b.dlgRef > 0 || s.__IfShowCancelDialogWhenImageTransfer == false || r == false)) {
						C.name = q;
						g.setBtnCancelVisibleForProgress(s, true);
						return g.httpPostUpload(s, o, C, r, 0, B, true, function(D) {
...
(function(a) {
	...
	var g = a.DOM,
		...
		f = {
			getServerImageUrlPrefix: function(n) {
				var l = n.httpUrl,
					k = false,
					m = [l, "dwt/dwt_", e.Trial ? "trial_" : "", a.replaceAll(e.ServerVersionInfo, ",", ""), "/img?id=", n.clientId];
				return m.join("")
			},
			output: function(k) {
				if (Dynamsoft.WebTwainEnv.Debug) {
	...
	d.OnReady = function() {
		var l = this,
			k = l.getView();
		k.BaseUrl = f.getServerImageUrlPrefix(l._stwain);
		return true
	};
	d.count = function() {
		var l = this,
	...
	d.GetCurrentImageIndex = function() {
		var l = this,
			k = l.getView();
		return k.GetCurrentImageIndex()
	};
	d.GetSelectedImageIndex = function(l) {
	...
	d.add = function(m, n, l) {
		var o = this,
			k = o.getView();
		return k.AddImage(m, n, l)
	};
	d.remove = function(m, l) {
		var n = this,
			k = n.getView();
		return k.RemoveImage(m, l)
	};
	d.MoveImage = function(m, l) {
		var n = this,
			k = n.getView();
		return k.MoveImage(m, l)
	};
	d.SwitchImage = function(m, l) {
		var n = this,
			k = n.getView();
		return k.SwitchImage(m, l)
	};
	d.GetImageWidth = function(o) {
		var r = this,
			p = o * 1,
			l, m, q, n, k;
		m = r.getView();
	...
	d.refresh = function(l, m, k, q) {
		var n = this;
		if (k) {
			var o = l * 1,
				p, t, r, s;
			t = n.getView();
			r = t.GetDataImageControlList();
			p = r.length;
			if (o < 0 || o > p) {
				return false
			}
			if (n._UIEditor && n._UIEditor._UIView) {
				n._UIEditor.refresh(o)
			}
			t.SetDataImageControl(o, q)
		}
		return true
	};
	d.refreshEditor = function(k, l) {
		var m = this;
		if (m._UIEditor) {
	...
	d.__Previous = function() {
		var l = this,
			k = l.getView();
		return k.__Previous()
	};
	d.SetSelectedImageCount = function(l) {
		var m = this,
			k = m.getView();
		return k.SetSelectedImageCount(l)
	};
	d.SetSelectedImageIndex = function(m, l) {
		var n = this,
			k = n.getView();
	...
	d.SetZoom = function(l) {
		var m = this,
			k = m.getView();
		return k.SetZoom(l)
	};
	d.PrintCurrent = function() {
		var p = this,
			l = p.getView();
		if (l.cCurrentIndex < 0) {
			return
		}
		var m = b.getImageUrl(l.BaseUrl, l.cCurrentIndex, -1, -1);
		var k = p.aryModelImageControl[l.cCurrentIndex];
		var o = [];
		var n = new a.UI.PrintUrl(m, k.imageWidth, k.imageHeight);
		o.push(n);
		b.print(o);
		return true
	};
	d.Print = function() {
		...
		for (i = 0; i < p.aryModelImageControl.length; i++) {
			var m = b.getImageUrl(l.BaseUrl, i, -1, -1);
	...
	d._isNoUI = function() {
		var k = this;
		if (k._stwain._bNoUI) {
			f.output("warning: cannot find UI.")
		}
		return k._stwain._bNoUI
	};
	d.ChangeSize = function(l, m) {
		var n = this,
			k = n.getView();
		return k.ChangeImageViewSize(l, m)
	};
	d.SetViewMode = function(m, l) {
		var n = this,
			k;
		if (n._isNoUI()) {
			return
	...
	d.ShowImageEditorEx = function(r, q, m, l, o) {
		var p = this;
		if (p._isNoUI()) {
			return
		}
		...
		s = {
			Container: a.one(k),
			aryModelImageControl: v.GetDataImageControlList(),
			BaseUrl: v.BaseUrl,
			cIndex: v.cCurrentIndex,
			stwain: p.getTwain()
		};
		u = new Dynamsoft.Lib.UI.ImageUIEditor(p, s);
		p._UIEditor = u;
		u.bShow = false;
		u.bShow = true;
		return u.ShowImageEditorEx(r, q, m, l, 0)
	};
	d.SetSelectionRectAspectRatio = function(m) {
		var p = this,
			n = m,
			k, o, l;
		p.__Ratio = n;
	...
	d.GetSelectionRectAspectRatio = function() {
		return this.__Ratio
	};
	d.GetSelectionImageBorderColor = function() {
		return this.__SelectionImageBorderColor
	};
	d.SetSelectionImageBorderColor = function(l) {
		var n = this,
			m = l,
			k;
		if (n._isNoUI()) {
	...
	d.GetBackgroundColor = function() {
		return this.__BackgroundColor
	};
	d.SetBackgroundColor = function(m) {
		var p = this,
			n = m,
			k, o, l;
		p.__BackgroundColor = n;
		if (p._isNoUI()) {
			return true
		}
		l = p.getView();
		...
		k = a.one("#" + p._stwain._strDWTControlContainerID);
		if (k) {
			o = k.one(".dynamsoft-dwt-container-box");
			if (o) {
	...
	d.GetAllowMultiSelect = function() {
		var l = this,
			k = l.getView();
		if (l._isNoUI()) {
			return false
	...
	d.SetAllowMultiSelect = function(l) {
		var m = this,
			k = m.getView();
		if (m._isNoUI()) {
			return true
	...
	d.SetShowPageNumber = function(l) {
		var m = this,
			k = m.getView();
		if (m._isNoUI()) {
			return true
	...
	d.OverlayRectangle = function(s, l, o, q, k, m, p) {
		var n = this,
			r = n.getView();
		if (n._isNoUI()) {
			return true
	...
	d.SetSelectedImageArea = function(n, p, o, m, l) {
		var q = this,
			k = q.getView();
		if (q._isNoUI()) {
			return true
		}
		return k.SetSelectedImageArea(index, p, o, m, l)
	};
...
(function(a) {
	...
	a.html5.CreateDWTObject = function(c, j, f, k) {
		var g, l, d, e, h;
		if (!f) {
			d = a.get(c);
			if (!d) {
		...
		for (e = 0; e < a.detect.arySTwains.length; e++) {
			h = a.detect.arySTwains[e];
			if (h._strDWTControlContainerID == c) {
				return false
			}
		}
		g = {
		...
		if (!f) {
			if (d.style.width) {
				g.width = d.style.width
			}
			if (d.style.height) {
				g.height = d.style.height
			}
		}
		if (j) {
			g.customIP = true;
		...
		l = new a.html5.STwain(g);
		if (j) {
			a.detect.arySTwainsByIP.push(l);
			return l
		}
		a.detect.arySTwains.push(l);
		if (a.detect.bFirst) {
			a._init(g)
		}
		a.detect.bFirst = false;
...
(function(b) {
	...
	c.prototype.onMouseDown = function(r) {
		var l = this,
			n = r.target,
			i = d,
			h, p, j, q, k, m, o;
		if (n == l.canvas || n == l.divArrow1 || n == l.divArrow2) {
			if (n == l.canvas) {
				h = b.DOM.getOffset(r);
				...
				if (n == l.divArrow1) {
					...
					if (n == l.divArrow2) {
						m = l.scrollSize + l.arrowSize
					}
				}
			}
			if (m <= l.arrowSize) {
				o = l._currentPosition;
				l.__click_previous();
				if (o != l._currentPosition) {
					l.__fireEvent("onValueChanged", {
						Value: l._currentPosition,
						EventName: "onPrevious"
					})
				}
			} else {
				if (m >= l.scrollSize + l.arrowSize) {
					o = l._currentPosition;
					l.__click_next();
					if (o != l._currentPosition) {
						l.__fireEvent("onValueChanged", {
		...
		if (i) {
			l.dragging = a;
			l.canvas.style.cursor = "default";
			l.mousePos_StartDrag.x = h.x;
			l.mousePos_StartDrag.y = h.y;
			l.mousePos_Dragging.x = h.x;
			l.mousePos_Dragging.y = h.y;
			var g, f, s = r || event;
			disX = s.clientX;
			disY = s.clientY;
			if (l.canvas.setCapture) {
				g = l.canvas;
				l.canvas.setCapture()
			} else {
				g = document
			...
			onMouseMove = function(v) {
				...
				w.x = l.mousePos_StartDrag.x + u.clientX - disX;
				w.y = l.mousePos_StartDrag.y + u.clientY - disY;
				e.output("onMouseMove: " + w.x + " " + w.y);
				if (l.dragging) {
					var t = l.__caculateDraggingNewPosition(w.x, w.y);
					if (t != l._currentPosition) {
						l.SetPosition(t);
						l.__fireEvent("onValueChanged", {
							Value: l._currentPosition,
			...
			onMouseUp = function(v) {
				...
				w.x = l.mousePos_StartDrag.x + u.clientX - disX;
				w.y = l.mousePos_StartDrag.y + u.clientY - disY;
				e.output("onMouseUp: " + w.x + " " + w.y);
				if (l.canvas.releaseCapture) {
					l.canvas.releaseCapture()
				}
	...
	c.prototype.__caculateDraggingNewPosition = function(f, l) {
		var k = this,
			g, i = 0,
			j = 0,
			h;
		g = k.__caculateSlideSize();
		slidePos = k.__caculateSlidePos();
		if (k.Horizontal) {
			startPos = f - k.arrowSize;
			j = f - k.mousePos_Dragging.x;
			h = f
		} else {
			startPos = l - k.arrowSize;
			j = l - k.mousePos_Dragging.y;
			h = l
		}
		e.output("__caculateDraggingNewPosition: " + [k._currentPosition, k._PagePositionSize, j, g].join(","));
		i = k._currentPosition + (k._PagePositionSize * j / g);
		if (i < 0) {
			i = 0
		}
		if (i > k._maxPosition) {
		...
		k.mousePos_Dragging.x = f;
		k.mousePos_Dragging.y = l;
		return i
	};
...
(function(b) {
	...
	e.prototype.ChangeControlSize = function(g, f) {
		...
		if (i.canvas) {
			if (i.bCanvas) {
				i.canvas.width = i.__controlWidth;
				i.canvas.height = i.__controlHeight
			} else {
				i.canvas.style.width = i.__controlWidth;
				i.canvas.style.height = i.__controlHeight
			}
		}
	};
	e.prototype.SetDrawImageLocationAndSize = function(i, h, g, f) {
		var j = this;
		d.output("SetLocationAndSize: " + [i, h, g, f].join(","));
		j.drawWidth = g;
		j.drawHeight = f;
		j.Left = i;
		j.Top = h;
		j.Right = j.Left + j.drawWidth;
		j.Bottom = j.Top + j.drawHeight
	};
	...
	e.prototype.__restoreCanvas = function(g, f, i) {
		var j = this;
		if (g) {
			g.canvas.width = f;
			g.canvas.height = i
	...
	e.prototype.__draw_dashRect = function(g, l, f, m) {
		var j = this,
			o = j.canvas.getContext("2d"),
			k = j.drawWidth,
			q = j.drawHeight,
			p = g,
			n = l,
		...
		d.output("__select_rect: " + [g, l, f, m].join(","));
		if (h < 0) {}
		if (h > k) {
			h = k
		}
		...
		o = j.canvas.getContext("2d");
		j.__restoreCanvas(o, o.canvas.width, o.canvas.height);
		if (h > 0 && i > 0) {
			o.__dashedLineRect(p + j.Left, n + j.Top, h, i)
		}
	};
	e.prototype.SetSelectedArea = function(h, p, q, g, j) {
		var n = this,
			s = n.canvas.getContext("2d"),
			u, r, f, m, i, k, o = n.drawWidth,
			t = n.drawHeight,
			l = false;
		d.output("SetSelectedArea: " + [h, p, q, g].join(","));
		if (!n.canvas) {
			return false
		}
		if (h < q) {
		...
		if (h > o || p > t) {
			...
			if (Math.abs(i) + Math.abs(k) <= 5) {
				l = true
			}
		}
		if (l) {
			u = n.selectedCanvasArea.left;
			r = n.selectedCanvasArea.top;
			f = n.selectedCanvasArea.right;
			m = n.selectedCanvasArea.bottom;
			if (u == 0 && r == 0 && f == 0 && m == 0) {} else {
				n.selectedCanvasArea.left = 0;
				n.selectedCanvasArea.top = 0;
				n.selectedCanvasArea.right = 0;
				n.selectedCanvasArea.bottom = 0;
				n.__restoreCanvas(s, s.canvas.width, s.canvas.height);
				if (j) {
					n.__fireEvent("onImageAreaDeSelected")
				}
			}
		} else {
			n.selectedCanvasArea.left = u;
			n.selectedCanvasArea.top = r;
			n.selectedCanvasArea.right = f;
			n.selectedCanvasArea.bottom = m;
			n.__draw_dashRect(u, r, i, k);
			if (j && (n.__mousedownStatus == 0)) {
				n.__fireEvent("onImageAreaSelected", {
					left: u,
					top: r,
					right: f,
					bottom: m,
	...
	e.prototype.ReFireSelectedEventWhenImageMoved = function() {
		var j = this,
			g, h, i, f;
		g = j.selectedCanvasArea.left;
		h = j.selectedCanvasArea.top;
		i = j.selectedCanvasArea.right;
		f = j.selectedCanvasArea.bottom;
		if (g != 0 && h != 0 && i != 0 && f != 0) {
	...
	e.prototype.SetMouseShape = function(j) {
		var l = this,
			f, h, i, k, g;
		if (l.canvas) {
			f = l.canvas.getContext("2d");
			if (j == EnumDWT_MouseShape.Hand) {
				l.canvas.style.cursor = "pointer";
				h = l.selectedCanvasArea.left;
				i = l.selectedCanvasArea.top;
				k = l.selectedCanvasArea.right;
				g = l.selectedCanvasArea.bottom;
				if (h == 0 && i == 0 && k == 0 && g == 0) {} else {
					l.SetSelectedArea(0, 0, 0, 0, true)
				}
			} else {
				if (j == EnumDWT_MouseShape.Crosshair) {
					l.canvas.style.cursor = "crosshair"
				} else {
					if (j == EnumDWT_MouseShape.Zoom) {
						if (b.env.bIE) {
							l.canvas.style.cursor = ""
						} else {
							l.canvas.style.cursor = "zoom-in"
						}
					} else {
						l.canvas.style.cursor = ""
					}
	...
	e.prototype.Destroy = function() {
		...
		d.output("Destroy:");
		if (j.canvas) {
			var f = j.canvas.getContext("2d");
			if (j.__controlWidth > 0 && j.__controlHeight > 0) {
				f.clearRect(0, 0, j.__controlWidth, j.__controlHeight)
			}
			i = j.canvas.parentNode;
			if (i) {
				i.removeChild(j.canvas)
			}
		}
	...
	e.prototype.__funReSelectArea = function(n) {
		var k = this,
			p, o, f, j, m, g = true;
		p = k.mouse_zero_x;
		o = k.mouse_zero_y;
		f = k.mouse_x;
		j = k.mouse_y;
		m = k.__GetSelectionRectAspectRatio();
		if (m > 0) {
			var i, h, l = 1;
			if (m < 1) {
				h = j - o;
		...
		k.SetSelectedArea(p, o, f, j, true)
	};
	e.prototype.BindEvents = function() {
		...
		Dynamsoft.Lib.addEventListener(g.canvas, "mousemove", function(i) {
			var h;
			h = b.DOM.getOffset(i, g);
			if (h.x < 0) {
				h.x = 0
			} else {
				if (h.x > g.drawWidth) {
		...
		Dynamsoft.Lib.addEventListener(g.canvas, "mousedown", function(k) {
			var j, i = new Date(),
				h;
			g.__mousedownStatus = 1;
			if (g.lastClick) {
				h = i.getTime() - g.lastClick.getTime()
			}
			g.lastClick = i;
			j = b.DOM.getOffset(k, g);
			if (j.x < 0) {
				j.x = 0
			} else {
				if (j.x > g.drawWidth) {
			...
			d.output("click on selector canvase: " + [j.x, j.y].join(","));
			g.SetSelectedArea(0, 0, 0, 0, true);
			g.mouse_zero_x = parseInt(j.x);
			g.mouse_zero_y = parseInt(j.y);
			if (h < 500) {
				...
				if (k.which ? k.which == 3 : k.button == 2) {
		...
		Dynamsoft.Lib.addEventListener(g.canvas, "mouseup", function(k) {
			var i;
			...
			if (g.__MouseShapeIsHand()) {
				...
				if (g.__MouseShapeIsZoom()) {
					if (Math.abs(j) < 3 && Math.abs(h) < 3) {
						...
						var m, l;
						m = (i.x + g.mouse_zero_x) / 2;
						l = (i.y + g.mouse_zero_y) / 2;
						g.__fireEvent("onZoom", {
							e: k,
							x: m,
							y: l,
					...
					var j = i.x - g.mouse_zero_x,
						h = i.y - g.mouse_zero_y;
					if (Math.abs(j) < 3 && Math.abs(h) < 3) {} else {
						g.__funReSelectArea();
	...
	e.prototype.__InvokeMouseUpOutside = function(i) {
		var j = this;
		var g;
		if (j.mouse_zero_x < 0 && j.mouse_zero_y < 0) {
			j.__mousedownStatus = 0;
			j.mouse_zero_x = -1;
			j.mouse_zero_y = -1;
			return
		}
		g = b.DOM.getOffset(i, j);
		if (g.x < 0) {
			g.x = 0
		} else {
			if (g.x > j.drawWidth) {
		...
		if (j.__mousedownStatus) {
			j.__mousedownStatus = 0;
			if (!j.__MouseShapeIsHand()) {
				var h = g.x - j.mouse_zero_x,
					f = g.y - j.mouse_zero_y;
				if (Math.abs(h) < 3 && Math.abs(f) < 3) {} else {
					j.__funReSelectArea(true)
				}
			}
		}
		j.mouse_zero_x = -1;
	...
	e.prototype.__isInSelectedArea = function(k) {
		var j = this,
			g, h, i, f;
		g = j.selectedCanvasArea.left;
		h = j.selectedCanvasArea.top;
		i = j.selectedCanvasArea.right;
		f = j.selectedCanvasArea.bottom;
		d.output("__deselectWhenMouseClick: " + [k.x, k.y, j.selectedCanvasArea.left, j.selectedCanvasArea.top, j.selectedCanvasArea.right, j.selectedCanvasArea.bottom].join(","));
		if (g == 0 && h == 0 && i == 0 && f == 0) {
			return false
		}
		if ((k.x >= g && k.x <= i) && (k.y >= h && k.y <= f)) {
			return true
		}
		return false
	};
...
(function(c) {
	...
	var b = true,
		...
		e = {
			...
			getAdjustedImageSize: function(n, i, o, j) {
				var l = {
						w: n,
						h: i,
						zoom: 1
					...
					f = Math.floor(o + 0.5),
					m = Math.floor(j + 0.5);
				e.output(["image info:", n, ",", i, "; max is ", f, ",", m].join(""));
				if (f <= n || m <= i) {
					if (f <= n && m <= i) {
						var g = f / n,
							k = m / i;
						if (g < k) {
							l.w = Math.floor(n * g + 0.5);
							l.h = Math.floor(i * g + 0.5);
							l.zoom = g
						} else {
							l.w = Math.floor(n * k + 0.5);
							l.h = Math.floor(i * k + 0.5);
							l.zoom = k
						}
					} else {
						if (f <= n) {
							l.zoom = f / n;
							l.w = Math.floor(f + 0.5);
							l.h = Math.floor(i * l.zoom + 0.5)
						} else {
							if (m <= i) {
								l.zoom = m / i;
								l.w = Math.floor(n * l.zoom + 0.5);
								l.h = Math.floor(m + 0.5)
							}
						}
					}
				}
				e.output(["change result:", l.w, ",", l.h].join(""));
				return l
			},
			getHex: function(g) {
				var f;
			...
			getColor: function(i) {
				...
				k = (i & 65280) >> 8;
				j = i & 255;
				h = ["#", e.getHex(j), e.getHex(k), e.getHex(f)].join("");
				return h
			},
	...
	a.prototype.__createSelector = function(f) {
		...
		if (!h.imageAreaSelector) {
			...
			h.imageAreaSelector.onMouseUp = function(i) {
				h.mousePreviosPositionWhenMoving.x = -1;
				h.mousePreviosPositionWhenMoving.y = -1
			};
			h.imageAreaSelector.onZoom = function(n) {
				var m, l, j, p, r = n.ddx,
					q = n.ddy,
					i;
				j = h.__getOrigImagePos();
				if (e.isNaNOrUndefined(j.x) || e.isNaNOrUndefined(j.y)) {
					return
				}
				p = h.__getRealZoomValue();
				if (p < 0) {
					return
				}
				m = (n.x + j.x) / p;
				...
				if (h.__checkZoomValue()) {
					i = h.fZoomInStep * h.fZoomValue;
					if (!h.__checkNewZoomValue(i)) {
						i = 25;
						k = true
					}
					if (h.fZoomValue != i) {
						h.__zoomInAt(i, m, l, n.x, n.y)
					} else {
						k = true
					}
			...
			h.imageAreaSelector.onMouseMove = function(p) {
				var m, l, w, v, k, j = p.dragging,
					t;
				k = h.__getOrigImagePos();
				if (e.isNaNOrUndefined(k.x) || e.isNaNOrUndefined(k.y)) {
					return
				}
				t = h.__getRealZoomValue();
				if (t < 0) {
					return
				}
				m = (p.x + k.x) / t;
				...
				if ((h.iMouseShape == EnumDWT_MouseShape.Hand) && j) {
					var s, r, q, n, i = h.__isShowScrollV(),
						u = h.__isShowScrollH();
					s = h.mousePreviosPositionWhenMoving.x;
					r = h.mousePreviosPositionWhenMoving.y;
					q = (s - p.x);
					n = (r - p.y);
					h.mousePreviosPositionWhenMoving.x = p.x;
					h.mousePreviosPositionWhenMoving.y = p.y;
					if (u && h.scrollH) {
						h.scrollH.ManualMoving(q)
					}
					if (i && h.scrollV) {
						h.scrollV.ManualMoving(n)
					}
				}
			};
			h.imageAreaSelector.onMouseClick = function(l) {
				var m, k, j, i;
				j = h.__getOrigImagePos();
				if (e.isNaNOrUndefined(j.x) || e.isNaNOrUndefined(j.y)) {
					return
				}
				i = h.__getRealZoomValue();
				if (i < 0) {
					return
				}
				m = (l.x + j.x) / i;
				k = (l.y + j.y) / i;
				h.mousePreviosPositionWhenMoving.x = l.x;
				h.mousePreviosPositionWhenMoving.y = l.y;
			...
			h.imageAreaSelector.onMouseDoubleClick = function(l) {
				var m, k, j, i;
				j = h.__getOrigImagePos();
				if (e.isNaNOrUndefined(j.x) || e.isNaNOrUndefined(j.y)) {
					return
			...
			h.imageAreaSelector.onMouseRightClick = function(l) {
				var m, k, j, i;
				j = h.__getOrigImagePos();
				if (e.isNaNOrUndefined(j.x) || e.isNaNOrUndefined(j.y)) {
					return
			...
			h.imageAreaSelector.onImageAreaSelected = function(p) {
				var l = p.left,
					n = p.top,
					q = p.right,
					k = p.bottom,
					i = p.fromScroll,
					m, j;
				e.output("selected Area in Selector: " + [l, n, q, k].join(","));
				if (h.fZoomValue > 0) {
					m = h.__getOrigImagePos();
					if (e.isNaNOrUndefined(m.x) || e.isNaNOrUndefined(m.y)) {
						return
					}
					l = (l + m.x);
					n = (n + m.y);
					q = (q + m.x);
					k = (k + m.y);
					e.output("selected zoomed Image Area: " + [l, n, q, k].join(","));
					...
					q = q / j;
					k = k / j;
					e.output("selected orig Image Area: " + [l, n, q, k].join(","));
					if (l < 0) {
						l = 0
					}
					if (n < 0) {
					...
					if (q > h.__getOrigImageWidth()) {
						q = h.__getOrigImageWidth()
					}
					if (k > h.__getOrigImageHeight()) {
						k = h.__getOrigImageHeight()
					}
	...
	a.prototype.ImageMoveHome = function() {
		var j = this,
			...
			i, f;
		if (g && j.scrollV) {
			f = j.scrollV.GetMaxPosition();
			j.scrollV.ManualMoving(-f)
		}
		if (h && j.scrollH) {
			f = j.scrollH.GetMaxPosition();
			j.scrollH.ManualMoving(-f)
		}
		return true
	};
	...
	a.prototype.__getOrigImagePos = function() {
		var j = this,
			...
			g = j.drawArea.height;
		if (f <= j.canvasWidth) {
			h = h + (j.canvasWidth - f) * 0.5
		}
		if (g <= j.canvasHeight) {
			i = i + (j.canvasHeight - g) * 0.5
		}
		return {
			x: h,
			y: i
	...
	a.prototype.__getImageDrawLocationAndSize = function(j, i) {
		var l = this,
			h = j,
			k = i,
			f, g;
		...
		if (e.isNaNOrUndefined(k)) {
			k = 0
		}
		f = l.drawArea.width;
		g = l.drawArea.height;
		if (f <= l.canvasWidth) {
			h = (l.canvasWidth - f) * 0.5;
			f = l.drawArea.width
		} else {
			h = h - l.drawArea.x;
			h = h < 0 ? 0 : h;
	...
	a.prototype.Show = function(j) {
		...
		e.output("Show:" + [i.canvasWidth, i.canvasHeight].join(","));
		f = i.canvas.getContext("2d");
		i.__restoreCanvas(f, i.canvasWidth, i.canvasHeight);
		if (i.drawArea.width <= 0 || i.drawArea.height <= 0 || i.canvasWidth <= 0 || i.canvasHeight <= 0) {
			return false
		}
		if (!j) {
			i.__resetScroll()
		...
		if (!i.objImage || i.objImage.src == "data:," || i.objImage.width == 0 || i.objImage.height == 0 || i.objImage.src == "") {
			f.fillStyle = i.backgroundColor;
		...
		e.output("draw info: " + [i.objImage.width, i.objImage.height, i.drawArea.x, i.drawArea.y, i.drawArea.width, i.drawArea.height].join(","));
		if (i.drawArea.width < i.canvasWidth || i.drawArea.height < i.canvasHeight) {
			f.fillStyle = i.backgroundColor;
			f.fillRect(0, 0, i.canvasWidth, i.canvasHeight)
		}
		var g = function() {
			try {
				if (i.objImage) {
					f.drawImage(i.objImage, Math.floor(-i.drawArea.x), Math.floor(-i.drawArea.y), Math.floor(i.drawArea.width), Math.floor(i.drawArea.height))
				}
				i.__overlay.Draw()
			} catch (k) {}
		};
	...
	a.prototype.SetSelectedImageArea = function(k, h, p, g) {
		var m = this,
			s = k,
			r = h,
			f = 0,
			l = 0,
			i = 1;
		e.output("SetSelectedImageArea:" + [k, h, p, g].join(", "));
		if (p < k && g < h) {
			s = parseInt(p);
			r = parseInt(g);
			f = parseInt(k);
		...
		m.selectedImageArea.left = s;
		m.selectedImageArea.top = r;
		m.selectedImageArea.right = f;
		m.selectedImageArea.bottom = l;
		if (m.imageAreaSelector) {
			...
			var j = s * i,
				...
				q = f * i,
				o = l * i;
			m.imageAreaSelector.SetSelectedArea(j, n, q, o);
			return true
		}
		return false
	};
	a.prototype.OverlayRectangle = function(l, k, n, h, f, j) {
		var m = this;
		e.output("OverlayRectangle:" + [l, k, n, h, f, j].join(", "));
		m.bHasOverlay = b;
		if (m._origImageWidth <= 0) {
			m.__getImageOrignalSize()
		}
		m.__refreshImageOverlayInfo();
		m.__overlay.Add(l, k, n, h, f, j, true);
		m.drawArea.x = 0;
		m.drawArea.y = 0;
		if (m.drawArea.width <= m.canvasWidth) {
			m.drawArea.x = (m.drawArea.width - m.canvasWidth) * 0.5
		} else {
			var i = Math.floor((l * m.fZoomValue + n * m.fZoomValue - m.canvasWidth) / 2 - m.drawArea.x);
			m.drawArea.x = i > 0 ? i : 0
		}
		if (m.drawArea.height <= m.canvasHeight) {
			m.drawArea.y = (m.drawArea.height - m.canvasHeight) * 0.5
		} else {
			var g = Math.floor((k * m.fZoomValue + h * m.fZoomValue - m.canvasHeight) / 2 - m.drawArea.y);
			m.drawArea.y = g > 0 ? g : 0
		}
		m.__setDrawArea(m.drawArea.x, m.drawArea.y, m.drawArea.width, m.drawArea.height);
		m.__resetScroll();
		m.Show();
		return true
	};
	...
	a.prototype.__setDrawArea = function(g, j, h, f) {
		var i = this;
		i.drawArea.x = g;
		i.drawArea.y = j;
		i.drawArea.width = h;
		i.drawArea.height = f;
		if (i.drawArea.width > i.canvasWidth && i.drawArea.x > i.drawArea.width - i.canvasWidth) {
			i.drawArea.x = i.drawArea.width - i.canvasWidth
		}
		if (i.drawArea.height > i.canvasHeight && i.drawArea.y > i.drawArea.height - i.canvasHeight) {
			i.drawArea.y = i.drawArea.height - i.canvasHeight
		}
		i.__refreshImageAreaSelectorLocationAndSize();
		i.__refreshImageOverlayInfo()
	};
	a.prototype.GetZoom = function() {
		var f = this;
		return f.fZoomValue
	};
	a.prototype.SetZoom = function(r, l, k, h, g) {
		var m = this,
			i, j = {},
			p, f, q;
		e.output("SetZoom:" + [i, r].join("->"));
		m.fUserSetZoom = r;
		j.x = m.drawArea.x;
		j.y = m.drawArea.y;
		j.fOldZoom = m.fZoomValue;
		j.fNewZoom = r;
		j.oldIfFitWindow = m.bIfFitWindow;
		j.oldFitWindowType = m.fitWindowType;
		j.oldCanvasWidth = m.canvasWidth;
		j.oldCanvasHeight = m.canvasHeight;
		j.bMoveCenter = false;
		if (!c.isUndefined(l) && !c.isUndefined(k)) {
			j.bMoveCenter = true;
			var o = (l / m.fZoomValue);
			var n = (k / m.fZoomValue);
			j.newRealCenterX = l;
			j.newRealCenterY = k;
			j.newClickX = h;
			j.newClickY = g
		}
		m.bIfFitWindow = false;
		m.fitWindowType = 3;
		m.fZoomValue = r;
		if (!m.objImage) {
			return
		}
		m.__recaculateDrawArea();
		p = m.__getScrollInfoWithNewZoom(j);
		m.__setDrawArea(p.x, p.y, m.drawArea.width, m.drawArea.height);
		m.__resetScroll();
		m.Show();
		return true
	};
	...
	a.prototype.__getScrollInfoWithNewZoom = function(C) {
		var y = this,
			E = {
				x: 0,
				y: 0
		...
		if (!C.bMoveCenter) {
			if (n) {
				h = t = y.objImage.width * 0.5;
				g = s = y.objImage.height * 0.5;
				m = t * p;
				k = s * p
			} else {
				h = h + w * 0.5;
			...
			m = C.newRealCenterX * p;
			k = C.newRealCenterY * p
		}
		bOldShowScrollV = y.__isShowScrollV(w), bOldShowScrollH = y.__isShowScrollH(i);
		x = y.__isShowScrollV(), F = y.__isShowScrollH();
		f = y.canvasWidth;
		q = y.canvasHeight;
		if (C.bMoveCenter) {
			var A = (y.drawArea.width - f) * 0.5,
				l = C.x,
				j = C.newRealCenterX * p,
				u = C.newRealCenterX * v,
				D = A - j - l + u;
			E.x = A - D;
			var r = (y.drawArea.height - q) * 0.5,
				z = C.y,
				j = C.newRealCenterY * p,
				u = C.newRealCenterY * v,
				D = r - j - z + u;
			E.y = r - D;
			return E
		}
	...
	a.prototype.__recaculateDrawArea = function() {
		var k = this,
			...
			f, h = false,
			i = false;
		k.canvasWidth = k._width;
		k.canvasHeight = k._height;
		if (k.objImage) {
			if (k.bIfFitWindow) {
				if (k.fitWindowType == 0) {
					f = e.getAdjustedImageSize(k.objImage.width, k.objImage.height, k.canvasWidth, k.canvasHeight);
					k.fZoomValue = f.zoom
				} else {
					if (k.fitWindowType == 1) {
						f = e.getAdjustedImageSize(k.objImage.width, k.objImage.height, k.objImage.width, k.canvasHeight);
						if (f.w > k._width) {
							h = true;
							k.canvasHeight = k._height - k.scrollArrowSize;
							f = e.getAdjustedImageSize(k.objImage.width, k.objImage.height, k.objImage.width, k.canvasHeight)
						}
						k.fZoomValue = f.zoom
					} else {
						if (k.fitWindowType == 2) {
							f = e.getAdjustedImageSize(k.objImage.width, k.objImage.height, k.canvasWidth, k.objImage.height);
							if (f.h > k._height) {
								h = true;
								k.canvasWidth = k._width - k.scrollArrowSize;
								f = e.getAdjustedImageSize(k.objImage.width, k.objImage.height, k.canvasWidth, k.objImage.height)
							}
							k.fZoomValue = f.zoom
						} else {
				...
				f = {
					w: k.objImage.width * k.fZoomValue,
					h: k.objImage.height * k.fZoomValue
				};
				j = true
			}
		} else {
		...
		if (g) {
			...
			k.drawArea.width = f.w;
			k.drawArea.height = f.h;
			if (k.drawArea.width < k.canvasWidth) {
				k.drawArea.x = (k.drawArea.width - k.canvasWidth) * 0.5
			} else {
				k.drawArea.x = 0
			}
			if (k.drawArea.height < k.canvasHeight) {
				k.drawArea.y = (k.drawArea.height - k.canvasHeight) * 0.5
			} else {
				k.drawArea.y = 0
			}
			e.output("draw Area: " + [k.drawArea.x, k.drawArea.y, k.drawArea.width, k.drawArea.height].join(","));
	...
	a.prototype.__resetScroll = function() {
		var h = this,
			j, g, f = h.__isShowScrollV(),
			k = h.__isShowScrollH(),
			o, i, n, l, m;
		if (h.canvas) {
			n = h.canvas.parentNode
		}
		if (!n) {
			return
		}
		...
		if (f) {
			...
			if (!h.scrollV) {
				...
				j.onValueChanged = function(p) {
					h.drawArea.y = p.Value;
				...
				h.scrollV = new Dynamsoft.Lib.UI.ScrollBar(j);
		...
		if (k) {
			...
			if (!h.scrollH) {
				...
				h.scrollH = new Dynamsoft.Lib.UI.ScrollBar(j);
	...
	a.prototype.__restoreCanvas = function(g, f, i) {
		var j = this;
		g.canvas.width = f;
		g.canvas.height = i
	};
	...
	a.prototype.__zoomInAt = function(g, f, k, i, h) {
		var j = this;
		if (j.fitWindowType == 4) {
			j.SetIfFitWindow(true, 0);
			j.__zoomIn()
		} else {
			j.SetZoom(g, f, k, i, h)
		}
...
(function(c) {
	...
	function h(k, j, q, r, m, i, s, p, l, o) {
		var n = this;
		n.iControlIndex = k;
		n.iImageIndex = j;
		n.View = q;
		n.imgControl = r;
		n.modelImageControl = m;
		n.iWidth = i;
		n.iHeight = s;
		n.AttachImagesForControl = p;
		n.bOnlyFromServer = l;
		n.BaseUrl = o
	}

	function b(n, m, l, j, i, k) {
		var o = this;
		o.Left = n;
		o.Top = m;
		o.Right = l;
	...
	function e(i) {
		...
		if (!Dynamsoft.WebTwainEnv.ScanDirectly) {
			j.bFocus = false;
			j.borderWidth = c.isUndefined(i.borderWidth) ? 1 : i.borderWidth;
			j.canvas = document.createElement("div");
			if (j.canvas) {
				j.canvas.style.width = i.Width + "px";
				j.canvas.style.height = i.Height + "px";
				if (j.borderWidth > 0) {
					j.canvas.style.border = j.borderWidth + "px solid #CCCCCC"
				}
				j.canvas.style.display = "block"
			}
			j.BaseUrl = false;
			i.Container.append(j.canvas);
			c.addEventListener(j.canvas, "mouseenter", function(k) {
				j.bFocus = true
			});
			c.addEventListener(j.canvas, "mouseleave", function(k) {
				j.bFocus = false
			});
			c.addEventListener(j.canvas, "mousewheel", function(k) {
				if (!j.bFocus) {
					return true
				}
				var l = c.getWheelDelta(k);
				if (l < 0) {
					j.__MousewheelValue--
				} else {
					if (l > 0) {
				...
				c.stopPropagation(k);
				setTimeout(function() {
					j.__MousewheelChanged(j)
				}, 200);
				return false
			});
			j.imageViewWidth = i.Width;
			j.imageViewHeight = i.Height;
			j.imagesPerRow = 1;
			j.imagesPerColumn = 1;
			j.MAX_ROWS = 128;
			j.MAX_COLUMNS = 128;
			...
			j.iMouseShape = EnumDWT_MouseShape.Default;
			j.bIfFitWindow = true;
			j.enumDWTFitWindowType = 0;
			j.fZoom = 1;
			j.zoomInStep = 1.2;
			j.zoomOutStep = 0.8;
			j.MIN_ZOOM = 0.02;
			...
			j.iControlHeight;
			j.iCurrentRow = -1;
			j.iOldFirstVisibleRow = 0;
			j.iCurrentColumn = -1;
			j.iOldFirstVisibleColum = 0;
			j.AllowMultiSelect = true;
			j.SelectionImageBorderColor = "#7DA2CE";
			j.BackgroundColor = "#FFFFFF";
			j.cCurrentIndex = -1;
			j.sHowManyImages = 0;
			j.bScrollBar = true;
			j.__InitiHorizontalScrollBar(i);
			j.__InitiVerticalScrollBar(i)
		}
		j.aryCurrentSelectedImageIndicesInBuffer = [];
		j.aryModelImageControl = i.aryModelImageControl;
		j.aryImageControls = [];
		j.bShowPageNumber = false;
		j.ratio = 0;
		j.__IfAutoScroll = true;
		j.__bAddingImage = false;
		j.__InitEvents(i);
		d.push(j);
		if (!Dynamsoft.WebTwainEnv.ScanDirectly) {
			if (j.canvas) {
				j.canvas.style.backgroundColor = j.BackgroundColor
			}
			j.__MousewheelValue = 0
		}
	}
	e.prototype.__InitEvents = function(i) {
		var j = this;
		if (c.isFunction(i.OnTopImageInTheViewChanged)) {
			j.OnTopImageInTheViewChanged = i.OnTopImageInTheViewChanged
		}
		if (c.isFunction(i.OnGetImageInfo)) {
			j.OnGetImageInfo = i.OnGetImageInfo
		}
		if (c.isFunction(i.onZoomChanged)) {
			j.onZoomChanged = i.onZoomChanged
		}
		if (c.isFunction(i.onMouseMove)) {
			j.onMouseMove = i.onMouseMove
		}
		if (c.isFunction(i.onMouseClick)) {
			j.onMouseClick = i.onMouseClick
		}
		if (c.isFunction(i.onMouseRightClick)) {
			j.onMouseRightClick = i.onMouseRightClick
		}
		if (c.isFunction(i.onMouseDoubleClick)) {
			j.onMouseDoubleClick = i.onMouseDoubleClick
		}
		if (c.isFunction(i.onImageAreaDeSelected)) {
			j.onImageAreaDeSelected = i.onImageAreaDeSelected
		}
		if (c.isFunction(i.onImageAreaSelected)) {
			j.onImageAreaSelected = i.onImageAreaSelected
		}
		if (c.isFunction(i.onSelected)) {
			j.onSelected = i.onSelected
		}
		if (c.isFunction(i.onRefreshUI)) {
			j.onRefreshUI = i.onRefreshUI
		}
		if (c.isFunction(i.onResetCurrentIndex)) {
			j.onResetCurrentIndex = i.onResetCurrentIndex
		}
	};
	e.prototype.__InitiHorizontalScrollBar = function(j) {
		var k = this;
		var i = {
			Width: j.Width,
			Height: 16,
		...
		i.onValueChanged = function(l) {
			k.__UpdateImageByScrollBar(l.Value)
		};
		k.horizontalScrollBar = new c.UI.ScrollBar(i);
		if (k.canvas && k.horizontalScrollBar) {
			k.canvas.appendChild(k.horizontalScrollBar.GetEL())
		}
		k.__SetHLocation(0, j.Height - k.__GetScrollWidth())
	};
	e.prototype.__InitiVerticalScrollBar = function(i) {
		var k = this;
		var j = {
			Width: 16,
			Height: i.Height,
		...
		j.onValueChanged = function(l) {
			k.__UpdateImageByScrollBar(l.Value)
		};
		k.verticalScrollBar = new c.UI.ScrollBar(j);
		if (k.canvas && k.verticalScrollBar) {
			k.canvas.appendChild(k.verticalScrollBar.GetEL())
		}
		k.__SetVLocation(i.Width - k.__GetScrollWidth(), 0)
	};
	e.prototype.__UpdateImageByScrollBar = function(l) {
		var o = this,
			i = -1,
			m = -1,
			k = -1,
			j, n;
		if (o.aryImageControls && o.aryImageControls.length > 0) {
			n = o.aryImageControls[0];
			if (n) {
				m = n.GetCurrentIndex();
				i = o.__GetRow(m)
			}
		...
		if (o.bVerticalMode == true) {
			k = parseInt(o.__GetVisibleRow(l) * o.imagesPerColumn);
			j = o.__GetRow(k);
			if (i == j) {
				return
			}
		} else {
			k = parseInt(o.__GetVisibleRow(l));
			if (k == m) {
				return
			}
		}
		f.output(o, "__UpdateImageByScrollBar(value, newIndex): " + [l, k].join(","));
		o.__GoInner(k, false, true)
	};
	e.prototype.UpdateImage = function(k, m, j, i, q, o) {
		var n = this;
		var r = n.aryModelImageControl[k];
		r.imageToShow = m;
		r.imageWidth = j;
		r.imageHeight = i;
		r.iImageControlWidth = j;
		r.iImageControlHeight = i;
		r.aryOverlayRectangleData = q;
		r.imageUrl = o;
		var p = n.GetImageControl(k);
		if (null != p) {
			p.SetCurrentIndex(k);
			if (m != null && (m.src == "data:," || m.width == 0 || m.height == 0 || m.src == "")) {
				var l = n.__GetImageControlIndex(k);
				if (l >= 0) {
					n.__SetArrayControlLoadImage(l, k, p, r, j, i, n.AttachImagesForControl, false)
				}
			} else {
				if (m == null) {
					var l = n.__GetImageControlIndex(k);
					if (l >= 0) {
						var j = parseInt(n.imageViewWidth / 2);
						var i = parseInt(n.imageViewHeight / 2);
						if (n.imagesPerRow == 1 && n.imagesPerColumn == 1) {
							if (n.bSingleMode == true) {
								j = -1;
								i = -1;
								r.bOriginalImage = true
							} else {
								j = n.imageViewWidth;
								i = n.imageViewHeight;
								r.bOriginalImage = false
							}
						} else {
							if (j < 300) {
								j = 300
						...
						n.__SetArrayControlLoadImage(l, k, p, r, j, i, n.AttachImagesForControl, true)
					}
				}
				n.AttachImagesForControl(p, r, k)
			}
		}
		n.iCurrentRow = -1;
		n.iCurrentColumn = -1;
	...
	e.prototype.__AddImage = function(j, k) {
		var l = this;
		l.__AddControl();
		var i = l.GetImageControl(j);
		i.SetCurrentIndex(j);
		l.AttachImagesForControl(i, k, j)
	};
	e.prototype.UpdateAllImage = function(m, i, k, j) {
		var l = this;
		return l.UpdateAllImageInner(m, i, false, k, j)
	};
	e.prototype.UpdateAllImageInner = function(q, j, n, m, l) {
		var p = this;
		p.aryModelImageControl = q;
		p.__OnlySetCurrentIndex(j);
		p.sHowManyImages = p.aryModelImageControl.length;
		var o = (p.imagesPerRow * p.imagesPerColumn) > p.sHowManyImages ? p.sHowManyImages : p.imagesPerRow * p.imagesPerColumn;
		var k = 0;
		for (k = p.aryImageControls.length - 1; k >= o; k--) {
			p.__RemoveControl(k)
		}
		for (k = p.aryImageControls.length; k < o; k++) {
			p.__AddControl()
		}
		if (!Dynamsoft.WebTwainEnv.ScanDirectly) {
			p.__ConfirmScrollMaxValue();
			p.__ConfirmScrollVisibity();
			p.iCurrentRow = -1;
			p.iCurrentColumn = -1;
			p.__SetCurrentIndex(p.cCurrentIndex);
			p.__ReInitChildrenPosition();
			p.__AttachImages(n, m, l)
		}
		return true
	};
	e.prototype.__AddControl = function() {
		var k = this,
			i = {};
		if (!Dynamsoft.WebTwainEnv.ScanDirectly) {
			if (c.isFunction(k.onImageAreaDeSelected)) {
				i.onImageAreaDeSelected = k.onImageAreaDeSelected
			}
			if (c.isFunction(k.onImageAreaSelected)) {
				i.onImageAreaSelected = k.onImageAreaSelected
			}
			i.onZoomChanged = function(l) {
				k.fZoom = l.zoom;
				k.__fireEvent("onZoomChanged", l)
			};
			i.onMouseClick = function(l) {
				k.__OnMouseClick(l);
				k.__fireEvent("onMouseClick", l)
			};
			i.onMouseRightClick = function(l) {
				k.__fireEvent("onMouseRightClick", l)
			};
			i.onMouseMove = function(l) {
				k.__fireEvent("onMouseMove", l)
			};
			i.onMouseDoubleClick = function(l) {
				k.__fireEvent("onMouseDoubleClick", l)
			};
			i.OnGetImageInfo = function(l) {
				return k.__fireEvent("OnGetImageInfo", l)
			}
		}
		var j = new c.UI.ImageControl(i);
		k.aryImageControls.push(j);
		c.imageControlCount++;
		k.__InitImageControl(j);
		if (!Dynamsoft.WebTwainEnv.ScanDirectly) {
			if (k.canvas) {
				k.canvas.appendChild(j.GetEL())
			}
			j.bindEvents()
		}
	};
	e.prototype.__InitImageControl = function(i) {
		var j = this;
		if (j.imagesPerRow == 1 && j.imagesPerColumn == 1) {
			i.SetMouseShape(j.iMouseShape);
			i.SetShowPageNumber(false)
		} else {
			i.SetMouseShape(EnumDWT_MouseShape.Hand);
			i.SetShowPageNumber(j.bShowPageNumber)
		}
		i.SetSelectionRectAspectRatio(j.ratio);
		i.SetBackgroundColor(j.BackgroundColor);
		i.SetSelectionImageBorderColor(j.SelectionImageBorderColor);
		if (j.bSingleMode) {
			if (j.bIfFitWindow) {
				i.SetIfFitWindow(true, j.enumDWTFitWindowType)
			} else {
				i.SetZoom(j.fZoom)
			}
		} else {
			i.SetIfFitWindow(true, 0)
		}
	};
	e.prototype.__OnMouseClick = function(k) {
		var n = this;
		var o = k.index;
		var r = n.aryCurrentSelectedImageIndicesInBuffer.indexOf(o),
			j = k.e.ctrlKey,
			p = k.e.shiftKey,
			q = k.e.metaKey;
		if (r == -1) {
			if (j || p || q || n.aryCurrentSelectedImageIndicesInBuffer.length < 1) {
				n.aryCurrentSelectedImageIndicesInBuffer.push(o)
			} else {
				if (n.aryCurrentSelectedImageIndicesInBuffer.length == 1) {
					if (n.aryCurrentSelectedImageIndicesInBuffer[0] == o) {
						return
					}
					n.aryCurrentSelectedImageIndicesInBuffer[0] = o
				} else {
			...
			if ((j || q) && n.aryCurrentSelectedImageIndicesInBuffer.length > 1) {
				n.aryCurrentSelectedImageIndicesInBuffer.splice(r, 1)
			} else {
				if (p) {
					n.aryCurrentSelectedImageIndicesInBuffer.push(o)
				} else {
					if (n.aryCurrentSelectedImageIndicesInBuffer.length == 1 && n.cCurrentIndex == o) {
						return
					}
					n.aryCurrentSelectedImageIndicesInBuffer = [];
					n.aryCurrentSelectedImageIndicesInBuffer.push(o)
		...
		if (p) {
			var u = n.aryCurrentSelectedImageIndicesInBuffer[0],
				m = n.aryCurrentSelectedImageIndicesInBuffer[n.aryCurrentSelectedImageIndicesInBuffer.length - 1];
			n.aryCurrentSelectedImageIndicesInBuffer = [];
			if (m == u) {
				n.aryCurrentSelectedImageIndicesInBuffer.push(parseInt(u))
			} else {
				if (m > u) {
					var s = m - u + 1;
					for (var l = 0; l < s; l++) {
						n.aryCurrentSelectedImageIndicesInBuffer.push(parseInt(u + l))
					}
				} else {
					var s = u - m + 1;
					n.aryCurrentSelectedImageIndicesInBuffer.push(parseInt(u));
					for (var l = 0; l < s - 1; l++) {
						n.aryCurrentSelectedImageIndicesInBuffer.push(parseInt(m + l))
					}
				}
			}
		}
		if (n.aryCurrentSelectedImageIndicesInBuffer.length == 1) {
			n.__OnlySetCurrentIndex(n.aryCurrentSelectedImageIndicesInBuffer[0]);
			n.__SetCurrentIndexWithoutSetScroll(n.cCurrentIndex)
		}
		n.__SortCurrentSelectedImageIndicesInBuffer();
		n.__SetUnselectAll();
		var l = 0;
		for (l = 0; l < n.aryCurrentSelectedImageIndicesInBuffer.length; l++) {
			var t = n.GetImageControl(n.aryCurrentSelectedImageIndicesInBuffer[l]);
			if (null != t) {
				t.SetSelect(true)
			}
		}
		n.__fireEvent("onSelected", n.aryCurrentSelectedImageIndicesInBuffer)
	};
	e.prototype.__SortCurrentSelectedImageIndicesInBuffer = function() {
		var p = this;
	...
	e.prototype.__RemoveControl = function(i) {
		var k = this;
		if (i < k.aryImageControls.length && i >= 0) {
			var j = k.aryImageControls[i];
			j.Destroy();
			k.aryImageControls.splice(i, 1);
			if (!Dynamsoft.WebTwainEnv.ScanDirectly) {
				if (k.canvas) {
					k.canvas.removeChild(j.GetEL())
				}
			}
			c.imageControlCount--;
			if (c.imageControlCount < 0) {
				c.imageControlCount = 0
			}
		}
	};
	e.prototype.__ConfirmScrollMaxValue = function() {
		var j = this;
		if (!j.bSingleMode) {
			if (j.bVerticalMode == true) {
				var i = j.__GetHowManyRows();
				j.__SetVScrollMaxValue(i, j.imagesPerRow)
			} else {
				j.__SetHScrollMaxValue(j.sHowManyImages, j.imagesPerColumn)
			}
		}
	};
	e.prototype.__GetHowManyRows = function() {
		var i = this;
		return parseInt((i.sHowManyImages + i.imagesPerColumn - 1) / i.imagesPerColumn)
	};
	e.prototype.__ConfirmScrollVisibity = function() {
		var i = this;
		if (i.bSingleMode) {
			i.__SetIfHScroll(false);
			i.__SetIfVScroll(false)
		} else {
			if (i.bVerticalMode == false) {
				if (i.bScrollBar && i.sHowManyImages > i.imagesPerRow * i.imagesPerColumn) {
					i.__SetIfHScroll(true)
				} else {
					i.__SetIfHScroll(false)
				}
				i.__SetIfVScroll(false)
	...
	e.prototype.__SetCurrentIndex = function(i) {
		var j = this;
		if (i >= 0) {
			j.__SetCurrentIndexInnerInner(i, true, false)
		}
	};
	e.prototype.__SetCurrentIndexInner = function(j, k, i) {
		var l = this;
		if (!i) {
			l.__SetCurrentIndexInnerInner(j, k, i)
		} else {
			l.__SetCurrentIndexInnerInner(j, k, true)
		}
	};
	e.prototype.__SetCurrentIndexWithoutSetScroll = function(i) {
		var j = this;
		j.__SetCurrentIndexInnerInner(i, true, false, true)
	};
	e.prototype.__SetCurrentIndexWithoutRefreshUI = function(i) {
		var j = this;
		j.__SetCurrentIndexInnerInner(i, true, false)
	};
	e.prototype.__SetCurrentIndexInnerInner = function(q, s, v, p) {
		var o = this,
			k = true;
		if (q < 0 || q >= o.aryModelImageControl.length) {
			return
		}
		if (o.__bAddingImage && !o.__IfAutoScroll) {
			k = false
		}
		if (k) {
			if (o.bVerticalMode == true) {
				var t = o.iCurrentRow;
				o.iCurrentRow = o.__GetRow(q);
				if (o.iCurrentRow != t && (o.iCurrentRow <= o.iOldFirstVisibleRow || ((o.iCurrentRow - o.iOldFirstVisibleRow) >= o.imagesPerRow))) {
					if ((o.iCurrentRow - o.iOldFirstVisibleRow) == o.imagesPerRow) {
						if (v == false) {
							o.iCurrentRow = o.iOldFirstVisibleRow + 1
						}
					}
					if (o.bSingleMode == false) {
						if (!p || p == false) {
							var n = o.iCurrentRow;
							o.__SetVScrollPotion(n)
						}
					}
					var l = o.__GetFirstVisibleRow();
					if (o.iOldFirstVisibleRow != l) {
						o.iOldFirstVisibleRow = l;
						var r = l * o.imagesPerColumn;
						o.__fireEvent("OnTopImageInTheViewChanged", r);
						o.__AttachImages(false)
					}
				}
			} else {
				var m = o.iCurrentColumn;
				o.iCurrentColumn = q;
				if (o.iCurrentColumn != m && (o.iCurrentColumn < o.iOldFirstVisibleColum || ((o.iCurrentColumn - o.iOldFirstVisibleColum) >= o.imagesPerColumn))) {
					if ((o.iCurrentColumn - o.iOldFirstVisibleColum) == o.imagesPerColumn) {
						o.iCurrentColumn = o.iOldFirstVisibleColum + 1
					}
					if (o.bSingleMode == false) {
						if (!p || p == false) {
							var j = o.iCurrentColumn;
							o.__SetHScrollPotion(j)
						}
					}
					var i = o.__GetFirstVisibleColumn();
					if (o.iOldFirstVisibleColum != i) {
						o.iOldFirstVisibleColum = i;
						var r = i;
						o.__fireEvent("OnTopImageInTheViewChanged", r);
						o.__AttachImages(false)
					}
		...
		if (s) {
			o.__SetUnselectAll();
			o.__OnlySetCurrentIndex(q);
			var u = o.GetImageControl(o.cCurrentIndex);
			if (null != u) {
				u.SetSelect(true)
			}
		}
	};
	e.prototype.__SetScrollByIndex = function(o) {
		var n = this;
		if (n.bVerticalMode == true) {
			var q = n.iCurrentRow;
			n.iCurrentRow = n.__GetRow(o);
			if (n.iCurrentRow != q && (n.iCurrentRow <= n.iOldFirstVisibleRow || ((n.iCurrentRow - n.iOldFirstVisibleRow) >= n.imagesPerRow))) {
				if ((n.iCurrentRow - n.iOldFirstVisibleRow) == n.imagesPerRow) {}
				if (n.bSingleMode == false) {
					var l = n.iCurrentRow;
					n.__SetVScrollPotion(l)
				}
				var k = n.__GetFirstVisibleRow();
				if (n.iOldFirstVisibleRow != k) {
					n.iOldFirstVisibleRow = k;
					var p = k * n.imagesPerColumn;
			...
			n.iCurrentColumn = o;
			if (n.iCurrentColumn != m && (n.iCurrentColumn < n.iOldFirstVisibleColum || ((n.iCurrentColumn - n.iOldFirstVisibleColum) >= n.imagesPerColumn))) {
				if ((n.iCurrentColumn - n.iOldFirstVisibleColum) == n.imagesPerColumn) {
					n.iCurrentColumn = n.iOldFirstVisibleColum + 1
				}
				if (n.bSingleMode == false) {
					var j = n.iCurrentColumn;
					n.__SetHScrollPotion(j)
				}
				var i = n.__GetFirstVisibleColumn();
				if (n.iOldFirstVisibleColum != i) {
					n.iOldFirstVisibleColum = i;
					var p = i;
					n.__fireEvent("OnTopImageInTheViewChanged", p);
					n.__AttachImages(false)
				}
	...
	e.prototype.__GetRow = function(i) {
		var j = this;
		return parseInt(i / j.imagesPerColumn)
	};
	e.prototype.__GetFirstVisibleColumn = function() {
		var i = this;
		return parseInt(i.__GetVisibleRow(i.__GetHScrollPosionValue()))
	};
	e.prototype.__GetFirstVisibleRow = function() {
		var i = this;
		return parseInt(i.__GetVisibleRow(i.__GetVScrollPosionValue()))
	};
	e.prototype.__GetVisibleRow = function(k) {
		var j = this;
		if (j.__GetHowManyRows() == 0) {
			return 0
		}
		var i = k;
		if (j.bSingleMode) {
			return j.cCurrentIndex
		} else {
			if (j.bVerticalMode == true) {
				if (j.__GetHowManyRows() - i < j.imagesPerRow) {
					i = i - (j.imagesPerRow - (j.__GetHowManyRows() - i))
				}
			} else {
				if (j.sHowManyImages - i < j.imagesPerColumn) {
					i = i - (j.imagesPerColumn - (j.sHowManyImages - i))
				}
			}
		}
		if (i < 0) {
	...
	e.prototype.__AttachImages = function(k, j, i) {
		var l = this;
		l.__AttachImagesInner(k, true, j, i)
	};
	e.prototype.__AttachImagesInner = function(p, q, j, l) {
		var o = this;
		if (o.aryImageControls && o.aryImageControls.length > 0) {
			var r = 0;
			if (o.bVerticalMode == true) {
				r = o.__GetFirstVisibleRow() * o.imagesPerColumn
			} else {
				r = o.__GetFirstVisibleColumn()
			}
			if (r < 0) {
				r = 0
			}
			var m = 0;
			var k = r;
			var u = r + o.aryImageControls.length - 1;
			if (j && j > 0) {
				k = j
			}
			if (l && l > 0) {
			...
			for (m = 0; m < o.aryImageControls.length; m++) {
				var s = o.aryImageControls[m];
				if (s) {
					var n = r + m;
					f.output(o, "__AttachImagesInner(baseIndex, index): " + [r, n].join(","));
					if (n < o.sHowManyImages) {
						if (p) {
							o.__InitImageControl(s)
						}
						if (q) {
							var t = o.aryModelImageControl[n];
							if (n >= k && n <= u || t.bIsDirty == true || s.GetCurrentIndex() != n) {
								f.output(o, "__AttachImagesInner() attach Image index -> control index:" + [n, m].join("->"));
								o.__GetImageAndAttach(s, t, n, false)
							}
						}
						s.SetVisible(true);
						f.output(o, "__AttachImagesInner() set control index Visible:" + [m].join());
						if (o.__IfContains(o.aryCurrentSelectedImageIndicesInBuffer, n)) {
							f.output(o, "__AttachImagesInner() set control index selected:" + [m].join());
							s.SetSelect(true)
						} else {
							s.SetSelect(false)
						}
					} else {
						f.output(o, "__AttachImagesInner() set control index InVisible:" + [m].join());
						s.SetVisible(false)
					}
				}
			}
	...
	e.prototype.__IfContains = function(l, k) {
		var j = 0;
		if (l) {
			for (j = 0; j < l.length; j++) {
				if (l[j] == k) {
	...
	e.prototype.__ReInitChildrenPosition = function() {
		var n = this,
			k = 0,
			m = 0,
			l;
		n.__InitControlsSize();
		if (!n.bSingleMode) {
			k = n.ImageMargin;
			m = n.ImageMargin
		}
		for (l = 0; l < n.aryImageControls.length; l++) {
			var j = n.aryImageControls[l];
			j.ChangeControlSize(n.iControlWidth, n.iControlHeight);
			if (n.bSingleMode) {
				j.SetLocation(0, 0)
			} else {
				j.SetLocation(k, m);
				k = j.Right + n.ImageMargin;
				if (n.bVerticalMode == true) {
					if ((l + 1) % n.imagesPerColumn == 0) {
						k = n.ImageMargin;
						m = j.Bottom + n.ImageMargin;
						if (n.bScrollBar == false) {
							if ((m + iControlHeight) > n.imageViewHeight) {
								m = n.ImageMargin
							}
						}
					}
	...
	e.prototype.__InitControlsSize = function() {
		var o = this;
		if (o.bSingleMode == false) {
			var j = o.__GetScrollWidth();
			if (o.bVerticalMode == true) {
				if (o.__GetHowManyRows() <= o.imagesPerRow || o.bScrollBar == false) {
					j = 0
				}
				if (o.ImageMargin <= 0 || (o.ImageMargin > (o.imageViewWidth - j - 3 * o.imagesPerColumn)) || (o.ImageMargin > (o.imageViewHeight - 3 * o.imagesPerRow))) {
					var m = parseInt(o.imageViewHeight / (o.imagesPerRow * 10));
					var n = parseInt((o.imageViewWidth - j) / (o.imagesPerColumn * 10));
					var l = m < n ? m : n;
					if (o.ImageMargin <= 0) {
						o.ImageMargin = parseInt(l)
					} else {
						o.ImageMargin = parseInt(o.ImageMargin < l ? o.ImageMargin : l)
					}
				}
			} else {
				if (o.sHowManyImages <= o.imagesPerColumn || o.bScrollBar == false) {
					j = 0
				}
				if (o.ImageMargin <= 0 || (o.ImageMargin > (o.imageViewWidth - 3 * o.imagesPerColumn)) || (o.ImageMargin > (o.imageViewHeight - j - 3 * o.imagesPerRow))) {
					var m = parseInt(o.imageViewHeight - j) / (o.imagesPerRow * 10);
					var n = parseInt((o.imageViewWidth) / (o.imagesPerColumn * 10));
					var l = m < n ? m : n;
					if (o.ImageMargin <= 0) {
						o.ImageMargin = parseInt(l)
					} else {
			...
			if (o.imagesPerColumn == o.imagesPerRow && o.imagesPerRow == 1) {
				o.ImageMargin = 0
			}
			var i = 0;
			var k = 0;
			if (o.bVerticalMode == true) {
				i = o.imageViewHeight - o.ImageMargin - 2 * o.borderWidth;
				k = o.imageViewWidth - j - o.ImageMargin - 2 * o.borderWidth
			} else {
				i = o.imageViewHeight - j - o.ImageMargin - 2 * o.borderWidth;
				k = o.imageViewWidth - o.ImageMargin - 2 * o.borderWidth
			}
			o.iControlWidth = parseInt(k / o.imagesPerColumn - o.ImageMargin);
			o.iControlHeight = parseInt(i / o.imagesPerRow - o.ImageMargin)
		} else {
			o.ImageMargin = 0;
			o.iControlWidth = o.imageViewWidth - 2 * o.borderWidth;
			o.iControlHeight = o.imageViewHeight - 2 * o.borderWidth
		}
	};
	e.prototype.GetDataImageControlList = function() {
		var i = this;
		return i.aryModelImageControl
	};
	e.prototype.SetDataImageControl = function(j, l) {
		var m = this;
		if (j > m.aryModelImageControl.length || m.aryModelImageControl.length <= 0) {
			return
		}
		var k;
		if (j == m.aryModelImageControl.length) {
			k = new c.UI.ModelImageControl();
			k.imageIndex = j;
			k.rawWidth = l;
			m.aryModelImageControl.push(k)
		} else {
			k = m.aryModelImageControl[j];
			k.rawWidth = l
		}
		if (k != null) {
			k.bIsDirty = true
		}
		var i = m.GetImageControl(j);
		if (null != i) {
			m.__GetImageAndAttach(i, k, j, true)
		}
		return true
	};
	e.prototype.AttachImagesForControl = function(i, k, j) {
		if (i.GetBindIndex() == j) {
			i.AttachImage(k.imageToShow, k.imageWidth, k.imageHeight, k.imageIndex, k.aryOverlayRectangleData, k.bOriginalImage);
			i.Show()
		}
		return true
	};
	e.prototype.__SetSelectionWinBorderColor = function(i) {
		var j = this
	};
	e.prototype.__SetUnselectAll = function() {
		var l = this;
		var k = 0;
		for (k = 0; k < l.aryImageControls.length; k++) {
			var j = l.aryImageControls[k];
			j.SetSelect(false)
		}
	};
	e.prototype.GetImageControl = function(k) {
		var l = this;
		var j = l.__GetImageControlIndex(k);
		if (j >= 0 && j < l.aryImageControls.length) {
			var i = l.aryImageControls[j];
			return i
		} else {
			return null
		}
	};
	e.prototype.__GetImageControlIndex = function(j) {
		var k = this;
		var i = -1;
		if (k.bSingleMode == true) {
			i = 0
		} else {
			if (k.bVerticalMode == true) {
				i = j - k.__GetFirstVisibleRow() * k.imagesPerColumn
			} else {
				i = j - k.__GetFirstVisibleColumn()
			}
		}
		if (i >= 0 && i < k.aryImageControls.length) {
			return i
		} else {
			return -1
		}
	};
	e.prototype.__SetIfHScroll = function(i) {
		var j = this;
		f.output(j, "__SetIfHScroll" + i);
		if (j.horizontalScrollBar) {
			j.horizontalScrollBar.SetVisible(i)
		}
	};
	e.prototype.__GetIfHScroll = function() {
		var i = this;
		if (i.horizontalScrollBar) {
			return i.horizontalScrollBar.GetVisible() == true ? true : false
		} else {
			return true
		}
	};
	e.prototype.__SetIfVScroll = function(i) {
		var j = this;
		f.output(j, "__SetIfVScroll" + i);
		if (j.verticalScrollBar) {
			j.verticalScrollBar.SetVisible(i)
		}
	};
	e.prototype.__GetIfVScroll = function() {
		var i = this;
		if (i.verticalScrollBar) {
			return i.verticalScrollBar.GetVisible() == true ? true : false
		} else {
			return true
		}
	};
	e.prototype.__GetHScrollPosionValue = function() {
		var j = this;
		var i = 0;
		if (j.horizontalScrollBar) {
			i = j.horizontalScrollBar.GetCurrentPosition()
		}
		if (i && i >= 0) {
			return i
		} else {
	...
	e.prototype.__GetVScrollPosionValue = function() {
		var j = this;
		var i = 0;
		if (j.verticalScrollBar) {
			i = j.verticalScrollBar.GetCurrentPosition()
		}
		if (i && i >= 0) {
			return i
		} else {
	...
	e.prototype.__SetHScrollMaxValue = function(j, i) {
		var k = this;
		f.output(k, "__SetHScrollMaxValue(totalSize,pageSize):" + j + "," + i);
		if (k.horizontalScrollBar) {
			k.horizontalScrollBar.InitScroll(j, i)
		}
	};
	e.prototype.__SetVScrollMaxValue = function(j, i) {
		var k = this;
		f.output(k, "__SetVScrollMaxValue(totalSize,pageSize):" + j + "," + i);
		if (k.verticalScrollBar) {
			k.verticalScrollBar.InitScroll(j, i)
		}
	};
	e.prototype.__SetHScrollPotion = function(i) {
		var j = this;
		if (j.horizontalScrollBar) {
			j.horizontalScrollBar.GoPosition(i)
		}
	};
	e.prototype.__SetVScrollPotion = function(i) {
		var j = this;
		if (j.verticalScrollBar) {
			j.verticalScrollBar.GoPosition(i)
		}
	};
	e.prototype.__SetHLocation = function(i, k) {
		var j = this;
		if (j.horizontalScrollBar) {
			j.horizontalScrollBar.SetLocation(i, k)
		}
	};
	e.prototype.__SetVLocation = function(i, k) {
		var j = this;
		if (j.verticalScrollBar) {
			j.verticalScrollBar.SetLocation(i, k)
		}
	};
	e.prototype.__ChangeHControlSize = function(i, j) {
		var k = this;
		if (k.horizontalScrollBar) {
			k.horizontalScrollBar.ChangeControlSize(i, j)
		}
	};
	e.prototype.__ChangeVControlSize = function(i, j) {
		var k = this;
		if (k.verticalScrollBar) {
			k.verticalScrollBar.ChangeControlSize(i, j)
		}
	};
	e.prototype.__GetScrollWidth = function() {
		return 16
	};
	e.prototype.SetSelectedImageArea = function(k, n, m, l, j) {
		var o = this;
		var i = o.GetImageControl(k);
		if (null != i) {
			if (o.imagesPerRow == 1 && o.imagesPerColumn == 1) {
				return i.SetSelectedImageArea(n, m, l, j)
			} else {
				return false
			}
		}
	...
	e.prototype.OverlayRectangle = function(r, n, m, q, k, j, l) {
		var o = this;
		var p = o.GetImageControl(r);
		if (null != p) {
			p.OverlayRectangle(n, m, q, k, j, l)
		}
		var s = o.aryModelImageControl[r];
		var i = new b(n, m, q, k, j, l);
		return s.aryOverlayRectangleData.push(i)
	};
	e.prototype.GetMousePosition = function() {
		var j = this;
		var i = j.GetImageControl(j.cCurrentIndex);
		if (null != i) {
			return i.GetMousePosition()
		} else {
			return null
		}
	};
	e.prototype.ChangeImageViewSize = function(i, j) {
		var k = this;
		if (i >= 0) {
			k.imageViewWidth = i;
			k.__ChangeHControlSize(i, 16);
			k.__SetVLocation(i - k.__GetScrollWidth(), 0)
		}
		if (j >= 0) {
			k.imageViewHeight = j;
			k.__ChangeVControlSize(16, j);
			k.__SetHLocation(0, j - k.__GetScrollWidth())
		}
		if (k.canvas) {
			k.canvas.style.width = i + "px";
			k.canvas.style.height = j + "px"
		}
		k.UpdateAllImage(k.aryModelImageControl, k.cCurrentIndex);
		return true
	};
	e.prototype.SetViewMode = function(r, k) {
		var n = this;
		var l = n.imagesPerColumn;
		var o = n.imagesPerRow;
		var s = n.bSingleMode;
		var j = n.bVerticalMode;
		if (r > 0) {
			n.imagesPerColumn = r
		}
		if (k > 0) {
		...
		if (r == -1 && k == -1) {
			...
			if (k == -1) {
				n.imagesPerRow = 1;
				n.bVerticalMode = false
			} else {
				n.bVerticalMode = true
			}
		}
		n.iOldFirstVisibleRow = 0;
		n.iOldFirstVisibleColum = 0;
		if (n.bSingleMode == false) {
			var m = 0;
			for (m = 0; m < n.aryImageControls.length; m++) {
				var p = n.aryImageControls[m];
				if (p != null) {
					p.SetOrigMode(false)
				}
			}
		}
		if ((l != n.imagesPerColumn || o != n.imagesPerRow) || (s != n.bSingleMode) || (j != n.bVerticalMode)) {
			if (n.imagesPerRow == 1 && n.imagesPerColumn == 1) {
				var m = 0;
				for (m = 0; m < n.aryImageControls.length; m++) {
					var p = n.aryImageControls[m];
			...
			if (n.aryCurrentSelectedImageIndicesInBuffer.length > 0) {
				var q = n.aryCurrentSelectedImageIndicesInBuffer[0];
				if (q >= 0 && q < n.aryModelImageControl.length) {
					n.cCurrentIndex = q
				}
			}
			n.UpdateAllImageInner(n.aryModelImageControl, n.cCurrentIndex, true)
		}
		n.__SetUnselectAll();
		n.__SetSelectedImageIndicesInBuffer();
		return true
	};
	e.prototype.RemoveAllImages = function() {
		return this.Clear()
	};
	e.prototype.Clear = function() {
		var m = this;
		var k = m.aryImageControls.length;
		for (j = k - 1; j >= 0; j--) {
			m.__RemoveControl(j)
		}
		var j = 0;
		for (j = 0; j < m.aryModelImageControl.length; j++) {
			m.aryModelImageControl[j].Clear()
		}
		m.aryModelImageControl = [];
		if (!Dynamsoft.WebTwainEnv.ScanDirectly) {
			m.__SetVScrollMaxValue(0, 0);
			m.__SetHScrollMaxValue(0, 0);
			m.__SetIfHScroll(false);
			m.__SetIfVScroll(false)
		}
		for (j = c.aryControlLoadImage.length - 1; j >= 0; j--) {
			var l = c.aryControlLoadImage[j];
			if (l.imgControl && l.View == m) {
				l.imgControl.SetBindIndex(-1);
				l.imgControl.Destroy();
				c.aryControlLoadImage.splice(j, 1)
			}
		}
		m.cCurrentIndex = -1;
		return true
	};
	e.prototype.ShowImage = function(j, k) {
		var l = this;
		l.__OnlySetCurrentIndex(j);
		if (l.bSingleMode == true) {
			var i = l.GetImageControl(l.cCurrentIndex);
			if (null != i) {
				l.__GetImageAndAttach(i, k, j, false)
			}
		}
		return true
	};
	e.prototype.__GetImageAndAttach = function(q, n, k, m) {
		var o = this;
		var j = parseInt(o.imageViewWidth / 2);
		var i = parseInt(o.imageViewHeight / 2);
		if (o.imagesPerRow == 1 && o.imagesPerColumn == 1) {
			if (o.bSingleMode == true) {
				j = -1;
				i = -1;
				n.bOriginalImage = true
			} else {
				j = o.imageViewWidth;
				i = o.imageViewHeight;
				n.bOriginalImage = false
			}
		} else {
			if (j < 300) {
				j = 300
		...
		if (m) {
			...
			if (n.bIsDirty == true || n.iImageControlWidth < j || n.iImageControlHeight < i || (n.iImageControlWidth > 0 && j == -1) || (n.iImageControlHeight > 0 && i == -1)) {
				p = true
			} else {
				p = false
			}
		...
		q.ClearImage();
		q.SetCurrentIndex(k);
		var l = o.__GetImageControlIndex(k);
		if (l >= 0) {
			o.__SetArrayControlLoadImage(l, k, q, n, j, i, o.AttachImagesForControl, p)
		}
	};
	e.prototype.__SetArrayControlLoadImage = function(m, l, s, p, k, j, r, n) {
		var q = this;
		var o = 0;
		for (o = 0; o < c.aryControlLoadImage.length; o++) {
			var t = c.aryControlLoadImage[o];
			if (t.iControlIndex == m && t.View == q) {
				t.iImageIndex = l;
				t.imgControl = s;
				t.modelImageControl = p;
				t.iWidth = k;
				t.iHeight = j;
				t.AttachImagesForControl = r;
				t.bOnlyFromServer = n;
				return
			}
		}
		c.aryControlLoadImage.push(new h(m, l, q, s, p, k, j, r, n, q.BaseUrl))
	};
	e.prototype.SetIfFitWindowAndType = function(j) {
		var k = this;
		if (k.bSingleMode == true) {
			k.bIfFitWindow = true;
			k.enumDWTFitWindowType = j;
			var i = k.GetImageControl(k.cCurrentIndex);
			if (null != i) {
				i.SetIfFitWindow(true, j);
				k.fZoom = i.GetZoom();
				i.Show()
			}
		} else {
	...
	e.prototype.GetAllowMultiSelect = function() {
		var i = this;
		return i.AllowMultiSelect
	};
	e.prototype.SetAllowMultiSelect = function(i) {
		var j = this;
		j.AllowMultiSelect = i;
		return true
	};
	e.prototype.GetFitWindowType = function() {
		var i = this;
		return i.enumDWTFitWindowType
	};
	e.prototype.SetSelectionRectAspectRatio = function(i) {
		var j = this;
		j.ratio = i;
		j.__AttachImagesInner(true, false);
		return true
	};
	e.prototype.SetFitWindowType = function(i) {
		var j = this;
		j.enumDWTFitWindowType = i;
		if (j.bSingleMode) {
			j.__AttachImagesInner(true, false)
		} else {
			j.bIfFitWindow = true;
	...
	e.prototype.GetIfFitWindow = function() {
		var i = this;
		return i.bIfFitWindow
	};
	e.prototype.SetIfFitWindow = function(i) {
		var j = this;
		j.bIfFitWindow = i;
		if (j.bSingleMode) {
			j.__AttachImagesInner(true, false)
	...
	e.prototype.GetImageMargin = function() {
		var i = this;
		return i.ImageMargin
	};
	e.prototype.SetImageMargin = function(i) {
		var j = this;
		j.ImageMargin = i;
		j.__ReInitChildrenPosition();
		j.__AttachImagesInner(false, false);
		return true
	};
	e.prototype.GetSelectionImageBorderColor = function() {
		var i = this;
		return i.SelectionImageBorderColor
	};
	e.prototype.SetSelectionImageBorderColor = function(i) {
		var j = this;
		j.SelectionImageBorderColor = i;
		j.__AttachImagesInner(true, false);
		return true
	};
	...
	e.prototype.SetZoom = function(k) {
		var j = this;
		if (k < j.MIN_ZOOM) {
			k = j.MIN_ZOOM
		}
		...
		j.fZoom = k;
		if (j.bSingleMode == true) {
			j.bIfFitWindow = false;
			j.enumDWTFitWindowType = 0;
			var i = j.GetImageControl(j.cCurrentIndex);
			if (null != i) {
				i.SetZoom(j.fZoom)
	...
	e.prototype.ZoomIn = function() {
		...
		if (m.bSingleMode == true) {
			var l = 15000;
			var j = m.aryModelImageControl[m.cCurrentIndex];
			var k = m.fZoom;
			if (m.bIfFitWindow) {
				var i = m.GetImageControl(m.cCurrentIndex);
				if (null != i) {
					k = i.GetZoom()
				}
			}
			tempZoom = (Math.floor(k * 100 * m.zoomInStep) + 1);
			if (1 < tempZoom && tempZoom < 2501) {
				if (parseInt(j.imageWidth * m.fZoom) > l || parseInt(j.imageHeight * m.fZoom) > l) {
					f.output(m, "You have reached the limit for zooming in");
					return false
				} else {
					m.SetZoom(tempZoom / 100)
				}
			} else {
				f.output(m, "You have reached the limit for zooming in");
	...
	e.prototype.ZoomOut = function() {
		...
		if (l.bSingleMode == true) {
			...
			var k = (Math.floor(j * 100 * l.zoomOutStep) - 1);
			if (1 < k && k < 6501) {
				l.SetZoom(k / 100)
			} else {
				f.output(l, "You have reached the limit for zooming out");
				return false
			}
		}
		return true
	};
	e.prototype.GetMouseShape = function() {
		var i = this;
		return i.iMouseShape
	};
	e.prototype.SetMouseShape = function(i) {
		var j = this;
		j.iMouseShape = i;
		j.__AttachImagesInner(true, false);
		return true
	};
	e.prototype.SetMouseShapeByIndex = function(j, k) {
		var l = this;
		l.__OnlySetCurrentIndex(j);
		l.iMouseShape = k;
		var i = l.GetImageControl(j);
		if (null != i) {
			i.SetMouseShape(l.iMouseShape);
			i.Show();
			return true
		}
		return false
	};
	e.prototype.GetShowPageNumber = function() {
		var i = this;
		return i.bShowPageNumber
	};
	e.prototype.SetShowPageNumber = function(i) {
		var j = this;
		j.bShowPageNumber = i;
		j.__AttachImagesInner(true, false);
		return true
	};
	e.prototype.SetBackgroundColor = function(i) {
		var j = this;
		j.BackgroundColor = i;
		if (j.canvas) {
			j.canvas.style.backgroundColor = i
		}
		j.__AttachImagesInner(true, false);
		return true
	};
	e.prototype.Go = function(i) {
		var j = this;
		j.__GoInner(i, true, false);
		return true
	};
	e.prototype.SetCurrentIndexOnly = function(i) {
		var j = this;
		return j.__GoInner(i, true, false)
	};
	e.prototype.__GoInner = function(n, q, r) {
		var l = this,
			p = l.cCurrentIndex;
		if (n < l.aryModelImageControl.length && n >= 0) {
			if (r == false) {
				l.__OnlySetCurrentIndex(n)
			}
			if (q == true) {
				if (r == true) {
					l.__SetCurrentIndex(l.cCurrentIndex)
				} else {
					l.__SetCurrentIndexWithoutRefreshUI(l.cCurrentIndex)
				}
			}
			if (r == true) {
				l.__AttachImages(false)
			}
			l.iCurrentRow = -1;
			l.iCurrentColumn = -1;
			if (q == false) {
				if (l.bVerticalMode == true) {
					var o = l.iCurrentRow;
					l.iCurrentRow = l.__GetRow(n);
					if (l.iCurrentRow != o && (l.iCurrentRow <= l.iOldFirstVisibleRow || (l.iCurrentRow > l.iOldFirstVisibleRow))) {
						var j = l.__GetFirstVisibleRow();
						if (l.iOldFirstVisibleRow != j) {
							l.iOldFirstVisibleRow = j;
							var m = j * l.imagesPerColumn;
							l.__fireEvent("OnTopImageInTheViewChanged", m)
						}
					}
				} else {
					var k = l.iCurrentColumn;
					l.iCurrentColumn = n;
					if (l.iCurrentColumn != k && (l.iCurrentColumn < l.iOldFirstVisibleColum || (l.iCurrentColumn > l.iOldFirstVisibleColum))) {
						var i = l.__GetFirstVisibleColumn();
						if (l.iOldFirstVisibleColum != i) {
							l.iOldFirstVisibleColum = i;
							var m = i;
							l.__fireEvent("OnTopImageInTheViewChanged", m)
						}
					}
	...
	e.prototype.SetSelectedImageCount = function(i) {
		var j = this;
		j.aryCurrentSelectedImageIndicesInBuffer.splice(i);
		j.__SetUnselectAll();
		j.__SetSelectedImageIndicesInBuffer();
		return true
	};
	e.prototype.SetSelectedImageIndex = function(n, m) {
		var p = this;
		p.aryCurrentSelectedImageIndicesInBuffer[n] = m;
		p.__SetUnselectAll();
		p.__SetSelectedImageIndicesInBuffer();
		var k = -1,
			j = p.aryCurrentSelectedImageIndicesInBuffer.length,
			o;
		for (var l = 0; l < j; l++) {
			var o = p.aryCurrentSelectedImageIndicesInBuffer[l];
			if (k == -1) {
				k = o
			} else {
				if (o < k) {
		...
		if (k >= 0 && k < p.aryModelImageControl.length && p.cCurrentIndex != k) {
			p.cCurrentIndex = k;
			p.__fireEvent("onResetCurrentIndex", p.cCurrentIndex)
		}
		return true
	};
	e.prototype.__SetSelectedImageIndicesInBuffer = function() {
		var m = this;
		if (m.aryCurrentSelectedImageIndicesInBuffer.length == 0) {
			if (m.cCurrentIndex >= 0) {
				m.aryCurrentSelectedImageIndicesInBuffer.push(m.cCurrentIndex)
			}
		}
		var k = 0;
		for (k = 0; k < m.aryCurrentSelectedImageIndicesInBuffer.length; k++) {
			var l = m.aryCurrentSelectedImageIndicesInBuffer[k];
			var j = m.GetImageControl(l);
			if (null != j) {
				j.SetSelect(true);
				j.Show()
			}
	...
	e.prototype.__OnlySetCurrentIndex = function(i) {
		var k = this,
			j = k.cCurrentIndex;
		k.cCurrentIndex = i;
		if (j != i) {
			k.__fireEvent("onResetCurrentIndex", i)
		}
		if (k.aryCurrentSelectedImageIndicesInBuffer[0] != k.cCurrentIndex) {
			k.aryCurrentSelectedImageIndicesInBuffer = [];
			k.aryCurrentSelectedImageIndicesInBuffer.push(k.cCurrentIndex)
		}
	};
	e.prototype.__MousewheelChanged = function(i) {
		if (i.__MousewheelValue > 0) {
			i.__MousewheelUp(i.__MousewheelValue)
		} else {
			if (i.__MousewheelValue < 0) {
				i.__MousewheelDown(-i.__MousewheelValue)
			}
		}
		i.__MousewheelValue = 0
	};
	e.prototype.__MousewheelUp = function(l) {
		var m = this;
		if (m.bSingleMode == false) {
			if (m.__GetIfHScroll() == true || m.__GetIfVScroll() == true) {
				...
				if (m.bVerticalMode == true) {
					j = m.__GetFirstVisibleRow() * m.imagesPerColumn;
					k = j - l * m.imagesPerColumn
				} else {
					j = m.__GetFirstVisibleColumn();
					k = j - l
				}
				if (k < 0) {
					k = 0
				}
				f.output(m, "__MousewheelUp(baseIndex, index): " + [j, k].join(","));
				m.__SetScrollByIndex(k)
			}
		} else {
			var i = m.GetImageControl(m.cCurrentIndex);
			if (null != i) {
	...
	e.prototype.__MousewheelDown = function(l) {
		var m = this;
		if (m.bSingleMode == false) {
			if (m.__GetIfHScroll() == true || m.__GetIfVScroll() == true) {
				var j = 0,
				...
				if (j + m.imagesPerColumn * m.imagesPerRow < m.aryModelImageControl.length) {
					f.output(m, "__MousewheelDown(baseIndex, index): " + [j, k].join(","));
					m.__SetScrollByIndex(k, true)
				}
			}
		} else {
			var i = m.GetImageControl(m.cCurrentIndex);
	...
	e.prototype.__Previous = function() {
		var i = this;
		i.Go(i.cCurrentIndex - 1);
		return true
	};
	e.prototype.__DoLeftArrow = function() {
		var j = this;
		if (j.bSingleMode == false) {
			j.Go(j.cCurrentIndex - 1)
		} else {
			var i = j.GetImageControl(j.cCurrentIndex);
			if (null != i) {
				i.ImageMoveLeft()
	...
	e.prototype.__Next = function() {
		var i = this;
		i.Go(i.cCurrentIndex + 1);
		return true
	};
	e.prototype.__DoRightArrow = function() {
		var j = this;
		if (j.bSingleMode == false) {
			j.__SetCurrentIndexInner(j.cCurrentIndex + 1, true)
		} else {
			var i = j.GetImageControl(j.cCurrentIndex);
			if (null != i) {
				i.ImageMoveRight()
	...
	e.prototype.__PreviousUp = function() {
		var k = this;
		if (k.bSingleMode == false) {
			var j = k.cCurrentIndex - k.imagesPerColumn;
			if (j >= 0) {
				k.Go(j)
			}
		} else {
	...
	e.prototype.__NextDown = function() {
		var k = this;
		if (k.bSingleMode == false) {
			var j = k.cCurrentIndex + k.imagesPerColumn;
			if (j < k.aryModelImageControl.length) {
				k.__SetCurrentIndexInner(j, true)
			}
		} else {
			var i = k.GetImageControl(k.cCurrentIndex);
			if (null != i) {
	...
	e.prototype.__PageUp = function() {
		var l = this;
		if (l.bSingleMode == false) {
			if (l.bVerticalMode == true) {
				var j = l.__GetFirstVisibleRow() * l.imagesPerColumn;
				var k = j - l.imagesPerColumn * l.imagesPerRow;
				if (k >= 0) {
					l.__SetCurrentIndexInner(k, false)
				} else {
	...
	e.prototype.__PageDown = function() {
		var l = this;
		if (l.bSingleMode == false) {
			if (l.bVerticalMode == true) {
				var j = l.__GetFirstVisibleRow() * l.imagesPerColumn;
				var k = j + l.imagesPerColumn * l.imagesPerRow;
				if (k < l.aryModelImageControl.length) {
					l.__SetCurrentIndexInner(k, false)
				} else {
					l.__SetCurrentIndexInner(l.aryModelImageControl.length - 1, false, false)
				}
			} else {}
		} else {
			var i = l.GetImageControl(l.cCurrentIndex);
	...
	e.prototype.__Home = function() {
		var j = this;
		if (j.bSingleMode == false) {
			j.__SetCurrentIndexInner(0, false)
		} else {
			var i = j.GetImageControl(j.cCurrentIndex);
			if (null != i) {
				i.ImageMoveHome()
	...
	e.prototype.__End = function() {
		var j = this;
		if (j.bSingleMode == false) {
			j.__SetCurrentIndexInner(j.aryModelImageControl.length - 1, false)
		} else {
			var i = j.GetImageControl(j.cCurrentIndex);
			if (null != i) {
				i.ImageMoveEnd()
	...
	e.prototype.__fireEvent = function(i, j) {
		var k = this;
		if (c.isFunction(k[i])) {
			return k[i](j)
		}
	...
	e.prototype.AddImage = function(m, k, q) {
		var n = this,
			o = m * 1,
			p, r, l, j;
		r = n.GetDataImageControlList();
		p = r.length;
		if (o < 0 || o > k) {
			return false
		}
		...
		n.__bAddingImage = true;
		n.UpdateAllImage(r, o, o, p);
		n.__bAddingImage = false;
		return true
	};
	e.prototype.RemoveImage = function(l, j) {
		var n = this,
			o = l * 1,
			q = j * 1,
			p, s, r;
		...
		n.UpdateAllImage(s, q, o, p - 2);
		return true
	};
	e.prototype.MoveImage = function(k, s) {
		var n = this,
			q, r, m, l, p, o, j;
		r = n.GetDataImageControlList();
		q = r.length;
		...
		n.UpdateAllImage(r, n.GetCurrentImageIndex(), o, j);
		return true
	};
	...
	e.prototype.HowManyImagesInBuffer = function() {
		return this.GetDataImageControlList().length
	};
	e.prototype.SwitchImage = function(k, j) {
		var o = this,
			i, n, m, l;
		n = o.GetDataImageControlList();
		i = n.length;
		if (k < n.length && k >= 0 && j < n.length && j >= 0) {
			m = n[k];
			m.imageIndex = j;
			l = n[j];
			l.imageIndex = k;
			n[j] = m;
			n[k] = l;
			o.UpdateImage(k, l.imageToShow, l.imageWidth, l.imageHeight, l.aryOverlayRectangleData, l.imageUrl);
			o.UpdateImage(j, m.imageToShow, m.imageWidth, m.imageHeight, m.aryOverlayRectangleData, m.imageUrl)
		}
		return true
	};
	e.prototype.GetIfAutoScroll = function() {
		return this.__IfAutoScroll
	};
	e.prototype.SetIfAutoScroll = function(i) {
		this.__IfAutoScroll = i ? true : false;
		return true
	};
	e.prototype.handlerKeyDownView = function(j) {
		var k = this,
			i = true;
		if (!k.bFocus) {
			return i
	...
	var a = function(j) {
		var i = true;
		c.each(d, function(k) {
			if (k instanceof c.UI.ImageUIView) {
				if (k.bFocus) {
					i = k.handlerKeyDownView(j);
					if (!i) {
						return false
					}
				}
	...
	c.addEventListener(document.documentElement, "keydown", a);
	c.aryControlLoadImage = [];
	c.imageControlCount = 0;
	c.AttachAndShowImage = function() {
		if (Dynamsoft.WebTwainEnv.ScanDirectly) {
			return
		}
		if (c.aryControlLoadImage.length == 0) {
			setTimeout(c.AttachAndShowImage, 500);
			return
		}
		var l = 0;
		var n = c.aryControlLoadImage[0];
		c.aryControlLoadImage.splice(0, 1);
		if (n.iControlIndex >= 0 && n.iControlIndex < c.imageControlCount) {
			var k = 2,
				j = n.iWidth,
				m = n.iHeight;
			if (j > k) {
			...
			if (n.bOnlyFromServer) {
				c.OnGetImageFromServer(n, n.modelImageControl, n.iImageIndex, j, m, n.BaseUrl, n.AttachImagesForControl)
			} else {
				c.OnGetImageByURL(n, n.modelImageControl, n.iImageIndex, j, m, n.BaseUrl, n.AttachImagesForControl)
			}
		} else {
			setTimeout(c.AttachAndShowImage, 0)
		}
	};
	c.OnGetImageByURL = function(r, k, n, p, m, o, j) {
		var i, q, l = r.imgControl;
		if (j) {
			if (k.imageUrl && k.imageUrl != "") {
				l.ClearImage();
				l.SetBindIndex(n);
				q = new Image();
				q.src = k.imageUrl;
				q.onload = function() {
					l.tmpImage = false;
					k.imageToShow = q;
					k.imageWidth = q.width;
					k.imageHeight = q.height;
					k.iImageControlWidth = p;
					k.iImageControlHeight = m;
					k.bIsDirty = false;
					j(l, k, n);
					q.onload = undefined;
					q.onerror = undefined;
				...
				q.onerror = function(t) {
					var u = l.GetBindIndex(),
						v = r.View,
						s;
					s = v.GetDataImageControlList().length;
					if (u >= 0 && u < s) {
						q.ready_src = k.imageUrl;
						setTimeout(function() {
							if (q.ready_src) {
								q.src = q.ready_src
							} else {
				...
				c.OnGetImageFromServer(r, k, n, p, m, o, j)
			}
		} else {
			setTimeout(c.AttachAndShowImage, 0)
		}
	...
	c.OnGetImageFromServer = function(r, k, n, p, m, o, j) {
		var i, q, l = r.imgControl;
		if (j && o) {
			l.ClearImage();
			l.SetBindIndex(n);
			i = f.getImageUrl(o, n, p, m);
			q = new Image();
			q.src = i;
			q.onload = function() {
				l.tmpImage = false;
			...
			q.onerror = function(t) {
				...
				s = v.GetDataImageControlList().length;
				if (u >= 0 && u < s) {
					q.ready_src = f.getImageUrl(o, l.GetBindIndex(), p, m);
					setTimeout(function() {
						if (q.ready_src) {
							q.src = q.ready_src
						} else {
...
(function(c) {
	...
	var g = "eng",
		...
		f = {
			...
			init: function(k, i) {
				var l = k,
					j = i || {},
					h = Math.floor(Math.random() * 100000 + 1);
			...
			__createButton: function(m, i, l, h, k) {
				var j = document.createElement("img");
				j.src = Dynamsoft.WebTwainEnv.ResourcesPath + "/reference/imgs/" + i + ".png";
				j.className = "dynamsoft-dwt-Editor_Buttons_" + i;
				j.title = l;
				j.style.width = h + "px";
				j.style.height = k + "px";
				this.__btnBindEvents(m, j, i);
				return j
			},
			__btnBindEvents: function(j, i, h) {
				switch (h) {
					case "previous":
						i.onclick = function() {
							j.previous_btn()
			...
			getToolBar: function(u, k, x) {
				var t = this,
					s;
				for (var p = 0; p < b.titlelan.length; p++) {
					if (b.titlelan[p] == g) {
						s = b.titles[p];
						break
					} else {
				...
				var m = document.createElement("ul"),
					w, r, q;
				var n = b.btninfo.length,
					v = false,
					l;
				var h = (k / n - 4) > (x - 4) ? (x - 4) : (k / n - 4);
				h = Math.floor(h);
				m.style.paddingTop = "2px";
				for (r = 0; r < n; r++) {
					w = document.createElement("li"), _w = h;
					w.style.floatLeft = "true";
					l = b.btninfo[r];
					if (l == "") {
						q = document.createElement("span");
						q.style.paddingLeft = h + "px";
						w.appendChild(q)
					} else {
						if (l == "hand" || l == "rectselect" || l == "zoom") {
							_w = h - 6;
							v = true
						} else {
							v = false
						}
						q = t.__createButton(u, l, s[r], _w, _w);
						if (v) {
							q.style.padding = "2px"
						}
						w.appendChild(q)
				...
				w.style["float"] = "right";
				var y = t.__createButton(u, "close", "Click to Close Window", h, h);
				y.onclick = function() {
					u.CloseImageEditor()
				};
				w.appendChild(y);
	...
	function a(k, i) {
		var m = this,
			j = i || {},
			h, l = '<div class="D_ImageUIEditor dynamsoft-dwt-noPaddingnoMarginInside dynamsoft-dwt-thinborder"></div>';
		m._UIManager = k;
		m.aryModelImageControl = m.__NewAryModelImageControl(i.aryModelImageControl);
		m._UIView;
		m.iControlWidth = 0;
		m.iControlHeight = 0;
		m._stwain = i.stwain;
		...
		h = m.container;
		h.append(l);
		m.UIEditor = h.one(".D_ImageUIEditor");
		m.bShow = false;
		m.zoom = 1;
		m.aspectRatio = 1;
		m.__fitWindowType = 0;
	...
	a.prototype.__NewAryModelImageControl = function(k) {
		var n = this;
		var l = [];
		var j = 0;
		for (j = 0; j < k.length; j++) {
			var h = new c.UI.ModelImageControl(),
				m = k[j];
			if (m.bOriginalImage) {
				h.bIsDirty = true;
				h.imageToShow = null;
				h.imageWidth = -1;
				h.imageHeight = -1;
				h.iImageControlWidth = -1;
				h.iImageControlHeight = -1
			} else {
				h.bIsDirty = m.bIsDirty;
				h.imageToShow = m.imageToShow;
				h.imageWidth = m.imageWidth;
				h.imageHeight = m.imageHeight;
				h.iImageControlWidth = m.iImageControlWidth;
				h.iImageControlHeight = m.iImageControlHeight
			}
			h.imageIndex = m.imageIndex;
			h.aryOverlayRectangleData = [];
			l.push(h)
		}
		return l
	};
	a.prototype.ShowImageEditorEx = function(i, l, h, k) {
		var j = this;
		h = window.innerWidth;
	...
	a.prototype.updateEditor = function() {
		var o = this;
		f.output(o, 'Editor is under mode "' + o.mode + '"');
		if (o.toolbar.self) {
			if (!o.bShow) {
				return
			}
			if (o.toolbar.width + o.toolbar.height == 0) {
				f.addClass(o.toolbar.self, "dynamsoft-dwt-notdisplayed")
			} else {
				...
				if (!o.toolbar.filled) {
					...
					o.toolbar.self.appendChild(f.getToolBar(o, o.toolbar.width, o.toolbar.height));
				...
				f.removeClass(o.toolbar.self, "dynamsoft-dwt-notdisplayed");
				f.removeClass(o.toolbar.self, "dynamsoft-dwt-overlay")
			}
			...
			l.id = "dynamsoft-dwt-viewerToolbar" + h;
			...
			var i = {
				...
				borderWidth: 0,
				aryModelImageControl: o.aryModelImageControl
			};
			i.onImageAreaSelected = function(s) {
			...
			i.onImageAreaDeSelected = function() {
			...
			o.toolbar.self = document.getElementById("dynamsoft-dwt-viewerToolbar" + h);
			o.updateEditor()
		}
	};
	a.prototype.updateMode = function(k) {
		var l = this,
			j;
		var i = c.one(l.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_next"),
			h = c.one(l.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_previous");
	...
	a.prototype.CloseImageEditor = function() {
		...
		if (h._IfImageChanged) {
			h.ShowDialogForSaveImage(-1, true);
			return
		}
		document.body.style.overflow = "";
		h.__HideImageEditorInner();
		h._UIManager._UIEditor = null;
		h._UIManager = null;
		h._stwain = null;
		h.evt_switch = null;
		h.evt_mouseMove = null;
		h.evt_mouseIn = null;
		h.evt_mouseOut = null;
		h.evt_mouseClick = null;
		h.evt_mouseDblClick = null;
		h.onRefreshUI = false;
		h.onMouseRightClick = false;
		h.onMouseClick = false;
		h.onMouseDoubleClick = false;
		h.onMouseMove = false
	};
	a.prototype.__HideImageEditorInner = function() {
		...
		h.aryModelImageControl = [];
		h.UIEditor = null;
		h.container.remove()
	};
	a.prototype.previous_btn = function() {
		var j = this,
			h, i = c.one(j.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_previous");
		if (i && i[0]) {
			h = i[0].src;
			if (h.indexOf("grey") != -1) {
				return
	...
	a.prototype.next_btn = function() {
		var j = this,
			h, i = c.one(j.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_next");
		if (i && i[0]) {
			h = i[0].src;
			if (h.indexOf("grey") != -1) {
				return
	...
	a.prototype.go = function(h) {
		var i = this;
		if (i._IfImageChanged) {
			i.ShowDialogForSaveImage(h, false);
			return
		}
		i.__goInner(h)
	};
	a.prototype.__goInner = function(k) {
		...
		var i = c.one(l.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_previous"),
			h = c.one(l.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_next");
		if (k == 0) {
			if (i && i[0]) {
				var j = i[0].src;
				j = j.replace("previous.", "previous_grey.");
	...
	a.prototype.Rotate = function(h, i) {
		var j = this;
		if (typeof h === "undefined" || typeof i === "undefined") {
			alert("Not enough paramters")
		} else {
			j._IfImageChanged = true;
			j._stwain.Rotate(j.cIndex, h, i)
		}
	};
	a.prototype.RotateEx = function(h, j, i) {
		var k = this;
		if (typeof h === "undefined" || typeof j === "undefined" || typeof i === "undefined") {
			alert("Not enough paramters")
		} else {
			k._IfImageChanged = true;
			k._stwain.RotateEx(k.cIndex, h, j, i)
		}
	};
	a.prototype.RotateLeft = function() {
		var h = this;
		h._IfImageChanged = true;
		h._stwain.RotateLeft(h.cIndex)
	};
	a.prototype.RotateAnyAngle = function(m) {
		...
		var i = c.get("dynamsoft_btnCancelRotateAnyAngle");
		i.onclick = function() {
			c.hide("dynamsoft_RotateAnyAngle")
		};
		var h = c.get("dynamsoft_RotateAnyAngle");
	...
	a.prototype.RotateRight = function() {
		var h = this;
		h._IfImageChanged = true;
		h._stwain.RotateRight(h.cIndex)
	};
	a.prototype.Deskew = function() {
		var h = this;
		h.RotateEx(h._stwain.GetSkewAngle(h.cIndex), true, 1)
	};
	a.prototype.Flip = function() {
		var h = this;
		h._IfImageChanged = true;
		h._stwain.Flip(h.cIndex)
	};
	a.prototype.Mirror = function() {
		var h = this;
		h._IfImageChanged = true;
		h._stwain.Mirror(h.cIndex)
	};
	a.prototype.ChangeImageSize = function(i, h, j) {
		var k = this;
		k._IfImageChanged = true;
		k._stwain.ChangeImageSize(k.cIndex, i, h, j)
	};
	a.prototype.ChangeImageSizeGetinput = function(m) {
		...
		q.options.add(new Option("Bicubic", 3));
		q = null;
		var h = c.get("dynamsoft_btnChangeImageSizeOK");
		h.onclick = function() {
		...
		var i = c.get("dynamsoft_ImgSizeEditor");
		...
		i.style.top = l + m.offsetHeight + document.documentElement.scrollTop + "px";
		i.style.left = n + document.documentElement.scrollLeft + "px";
	...
	a.prototype.Crop_btn = function() {
		var j = this,
			h, i = c.one(j.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_crop");
		if (i && i[0]) {
			h = i[0].src;
			if (h.indexOf("grey") != -1) {
				return
	...
	a.prototype.Erase_btn = function() {
		var j = this,
			i, h = c.one(j.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_erase");
		if (h && h[0]) {
			i = h[0].src;
			if (i.indexOf("grey") != -1) {
				return
	...
	a.prototype.Crop = function() {
		...
		var i = l.Right;
		var h = l.Bottom;
		if (parseInt(k) - parseInt(i) != 0 && parseInt(j) - parseInt(h) != 0) {
			l._IfImageChanged = true;
			l._stwain.Crop(l.cIndex, parseInt(k), parseInt(j), parseInt(i), parseInt(h));
			l.__SetCropButtonGrey(true);
			l.__SetEraseButtonGrey(true)
		} else {
			f.output(l, "No area selected for cropping");
			return
		}
	};
	a.prototype.Erase = function() {
		...
		if (parseInt(k) - parseInt(i) != 0 && parseInt(j) - parseInt(h) != 0) {
			l._IfImageChanged = true;
			l._stwain.Erase(l.cIndex, parseInt(k), parseInt(j), parseInt(i), parseInt(h));
			l.__SetEraseButtonGrey(true)
		} else {
			f.output(l, "No area selected for cropping");
			return
	...
	a.prototype.StrechMode = function() {
		var h = this;
		if (h._UIView) {
			h._UIView.SetIfFitWindowAndType(4)
		}
	...
	a.prototype.FitsWindowSize = function() {
		var h = this;
		if (h._UIView) {
			h._UIView.SetIfFitWindowAndType(0)
		}
		h.__SetZoomInButtonEnable(true);
		h.__SetZoomOutButtonEnable(true);
		h.__SetCropButtonGrey(true);
	...
	a.prototype.FitsWindowWidth = function(h) {
		var i = this;
		if (i._UIView) {
			i._UIView.SetIfFitWindowAndType(2)
		}
		i.__SetZoomInButtonEnable(true);
		i.__SetZoomOutButtonEnable(true);
		i.__SetCropButtonGrey(true);
		i.__SetEraseButtonGrey(true);
		return
	};
	a.prototype.FitsWindowHeight = function(h) {
		var i = this;
		if (i._UIView) {
			i._UIView.SetIfFitWindowAndType(1)
		}
		i.__SetZoomInButtonEnable(true);
		i.__SetZoomOutButtonEnable(true);
		i.__SetCropButtonGrey(true);
	...
	a.prototype.ZoomIn = function() {
		var h = this;
		if (h._UIView && h._UIView.ZoomIn()) {
			h.__SetZoomInButtonEnable(true);
			h.__SetZoomOutButtonEnable(true)
		} else {
			h.__SetZoomInButtonEnable(false);
			h.__SetZoomOutButtonEnable(true)
		}
		h.__SetCropButtonGrey(true);
		h.__SetEraseButtonGrey(true)
	...
	a.prototype.ZoomOut = function() {
		var h = this;
		if (h._UIView && h._UIView.ZoomOut()) {
			h.__SetZoomOutButtonEnable(true);
			h.__SetZoomInButtonEnable(true)
		} else {
			h.__SetZoomOutButtonEnable(false);
			h.__SetZoomInButtonEnable(true)
		}
		h.__SetCropButtonGrey(true);
		h.__SetEraseButtonGrey(true)
	...
	a.prototype.Hand_btn = function() {
		var i = this,
			h = EnumDWT_MouseShape.Hand;
		if (i._UIView) {
			i._UIView.SetMouseShapeByIndex(i.cIndex, h)
		}
		i.__EnableMouseShapButtons(h)
	};
	a.prototype.RectSelect_btn = function() {
		var i = this,
			h = EnumDWT_MouseShape.Crosshair;
		if (i._UIView) {
			i._UIView.SetMouseShapeByIndex(i.cIndex, h)
		}
		i.__EnableMouseShapButtons(h)
	...
	a.prototype.__EnableMouseShapButtons = function(o) {
		var j = this,
			...
			p, m, k;
		if (n) {
			var h = n.one(".dynamsoft-dwt-Editor_Buttons_hand"),
				i = n.one(".dynamsoft-dwt-Editor_Buttons_rectselect"),
				l = n.one(".dynamsoft-dwt-Editor_Buttons_zoom");
			f.removeClass(h.parent(), "dynamsoft-dwt-EditorButton-selected");
			f.removeClass(i.parent(), "dynamsoft-dwt-EditorButton-selected");
			f.removeClass(l.parent(), "dynamsoft-dwt-EditorButton-selected");
			if (o == EnumDWT_MouseShape.Hand) {
				f.addClass(h.parent(), "dynamsoft-dwt-EditorButton-selected")
			} else {
				if (o == EnumDWT_MouseShape.Crosshair) {
					f.addClass(i.parent(), "dynamsoft-dwt-EditorButton-selected")
				} else {
					if (o == EnumDWT_MouseShape.Zoom) {
						f.addClass(l.parent(), "dynamsoft-dwt-EditorButton-selected")
					}
				}
			}
		}
	...
	a.prototype.print = function() {
		var h = this;
		if (h._UIView) {
			h._UIView.PrintCurrent()
		}
	};
	a.prototype.RestoreImage = function() {
		var h = this;
		h.__RestoreImageInner(true, -1)
	};
	a.prototype.__RestoreImageInner = function(k, i, n) {
		var o = this,
			l;
		if (o.cIndex == -1) {
			if (n) {
		...
		if (o._UIView && o._IfImageChanged == true) {
			var h = o._UIView.GetImageControl(o.cIndex);
			if (null != h) {
				h.SetCurrentIndex(o.cIndex);
				o.OnGetImageFromServerForEdit(h, o.aryModelImageControl[o.cIndex], o.cIndex, 1, j);
				m = false
			}
		}
		if (m) {
	...
	a.prototype.OnGetImageFromServerForEdit = function(h, m, k, l, n) {
		var o = this,
			j, i;
		if (n && o.BaseUrl) {
			h.ClearImage();
			h.SetBindIndex(k);
			j = d.getUrlByAct(o.BaseUrl, k, l);
			i = new Image();
			i.src = j;
			i.onload = function() {
				h.tmpImage = false;
				m.imageToShow = i;
				m.imageWidth = i.width;
				m.imageHeight = i.height;
				m.imageUrl = j;
				m.bIsDirty = false;
				n(h, m, k);
				i.onerror = undefined;
			...
			i.onerror = function(r) {
				var s = h.GetBindIndex(),
					q = o._stwain,
					p = q.HowManyImagesInBuffer;
				if (s >= 0 && s < p) {
					i.ready_src = d.getImageUrl(o.BaseUrl, h.GetBindIndex(), l);
					setTimeout(function() {
						if (i.ready_src) {
							i.src = i.ready_src
						} else {
			...
			if (h.tmpImage) {
				h.tmpImage.src = "data:,";
				h.tmpImage.onload = undefined
			}
			h.tmpImage = i
		}
		return true
	};
	a.prototype.ChangeSize = function(i, j) {
		var k = this;
		if (i >= 0) {
			k.containerWidth = i
		}
	...
	a.prototype.SaveImage = function() {
		var h = this;
		h.__SaveImageInner(true, function() {
			h._stwain._OnBitmapChanged(["0", h.cIndex.toString(), 4, h.cIndex, h._UIView.sHowManyImages])
		})
	};
	a.prototype.__SaveImageInner = function(j, m) {
		...
		if (n._UIView && n._IfImageChanged == true) {
			var h = n._UIView.GetImageControl(n.cIndex);
			if (null != h) {
				h.SetCurrentIndex(n.cIndex);
				n.OnGetImageFromServerForEdit(h, n.aryModelImageControl[n.cIndex], n.cIndex, 2, i);
				l = false
			}
		}
		if (l) {
	...
	a.prototype.ShowDialogForSaveImage = function(i, l) {
		var m = this;
		...
		var h = c.one(".dynamsoft_btnSave").getEL();
		if (h) {
			h.onclick = function() {
		...
		if (k) {
			k.onclick = function() {
				m.__RestoreImageInner(false, i, function() {
					var n = m._stwain._strDWTControlContainerID;
					DCP_DWT_OnClickCloseInstall(n);
					c.bChangeImage = 0;
					if (i == -1) {
	...
	a.prototype.__SetCropButtonGrey = function(j) {
		...
		if (k && k[0]) {
			var i = k[0].src;
			i = i.replace("crop.", "crop_grey.");
			k[0].src = i;
			k.style("cursor", "auto")
		}
		if (l._UIView && j) {
			var h = l._UIView.GetImageControl(l.cIndex);
			if (null != h) {
				h.SetSelectedImageArea(0, 0, 0, 0)
			}
		}
	};
	a.prototype.__SetEraseButtonGrey = function(k) {
		...
		l.Right = 0;
		l.Bottom = 0;
		var i = c.one(l.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_erase");
		if (i && i[0]) {
			var j = i[0].src;
	...
	a.prototype.__SetZoomInButtonEnable = function(h) {
		var k = this;
		var j = c.one(k.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_zoomin");
		if (j && j[0]) {
			var i = j[0].src;
			if (h == true) {
				i = i.replace("zoomin_grey.", "zoomin.");
				j[0].src = i;
				j.style("cursor", "pointer")
			} else {
				i = i.replace("zoomin.", "zoomin_grey.");
				j[0].src = i;
				j.style("cursor", "auto")
			}
		}
	};
	a.prototype.__SetZoomOutButtonEnable = function(h) {
		var k = this;
		var j = c.one(k.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_zoomout");
		if (j && j[0]) {
			var i = j[0].src;
			if (h == true) {
				i = i.replace("zoomout_grey.", "zoomout.");
				j[0].src = i;
				j.style("cursor", "pointer")
			} else {
				i = i.replace("zoomout.", "zoomout_grey.");
				j[0].src = i;
				j.style("cursor", "auto")
			}
		}
	...
	a.prototype.refresh = function(j) {
		...
		if (k._IfImageChanged) {
			var h = c.one(k.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_save"),
				i = c.one(k.toolbar.self).one(".dynamsoft-dwt-Editor_Buttons_restore");
			if (h && h[0]) {
				h[0].style.display = "";
				i[0].style.display = ""
			}
...
(function(a, H) {
	var K = dynamsoft.navInfo,
		Q = dynamsoft.dcp,
		O = !1,
		T = "The Dynamic Web TWAIN module is not installed.",
		w = {
			curPortIndex: 0,
			bConnected: false,
			versionInfo: function(Z, Y) {
				var X = 0,
					V = function(af, ac) {
						var ab = [w._getPreUrl(Z[w.curPortIndex]), "f/VersionInfo?ts=", a.now()].join(""),
							aa, ae;
						aa = [H.Trial ? "dwt_trial_" : "dwt_", a.replaceAll(H.ServerVersionInfo, ",", "")].join("");
						ae = a.stringify({
							id: "1",
							method: "VersionInfo",
							version: aa,
						...
						if (H.OnWebTwainInitMessage) {
							var ad = "checking WebTwain version ...";
							H.OnWebTwainInitMessage(ad, EnumDWT_InitMsg.Info)
						}
						a.ajax({
							method: "POST",
							url: ab,
					...
					W = function() {
						w.curPortIndex++;
						w.bConnected = false;
						if (w.curPortIndex < Z.length) {
							setTimeout(function() {
								V(U, W)
							}, 1000)
						} else {
							H.OnWebTwainInitMessage && H.OnWebTwainInitMessage(T, EnumDWT_InitMsg.NotInstalledError);
					...
					U = function(ab) {
						var aa = Q._getJSON(ab);
						if (aa && "result" in aa) {
							aa = aa.result;
							if (a.isArray(aa) && aa.length > 0) {
								Y && setTimeout(function() {
									var ac = Z[w.curPortIndex],
										af = aa[0],
										ad = aa.length > 1 ? aa[1] : "";
									if (ad !== "") {
										H.OnWebTwainInitMessage && H.OnWebTwainInitMessage(ad, EnumDWT_InitMsg.Error)
									} else {
										var ae = "The WebTwain version is: " + af;
										H.OnWebTwainInitMessage && H.OnWebTwainInitMessage(ae, EnumDWT_InitMsg.Info)
									}
									Y(ac, af, ad)
								}, 0);
								return
			...
			_getPreUrl: function(U) {
				...
				return [V, dynamsoft.dcp.ip, ":", U, "/"].join("")
			}
		};
	a.DynamicWebTwain = O;
	var J = window,
		n = J.document,
		k, o, R = "This browser is currently not supported.",
		C = "Duplicate ID detected for creating Dynamic Web TWAIN objects, please check and modify.",
		L = a.each,
		f = "dwt",
		j = 0,
		E = 1,
	...
	a.__innerInitEvents = function(V, U) {
		U && L(["onAfterOperate", "onBeforeOperate", "onOperateStatus", "onBitmapChanged", "onGetFilePath", "onImageAreaDeSelected", "onImageAreaSelected", "onMouseClick", "onMouseDoubleClick", "onMouseMove", "onMouseRightClick", "onPreAllTransfers", "onPreTransfer", "onPostLoad", "onPostTransfer", "onPostAllTransfers", "onSourceUIClose", "onTopImageInTheViewChanged", "onTransferCancelled", "onTransferError", "onInternetTransferPercentage", "onInternetTransferPercentageEx"], function(W) {
			V["__O" + W.substring(1)] = U[W] || ""
		})
	};
	a.replaceControl = function(V, U) {
		H.ContainerMap[V] = U
	};
	k = function(af, X, ac, U) {
		var V = (navigator.userAgent.toLowerCase()),
			ae = a.product;
		if (U && U._customIP && (!a.isLocalIP(U._ip))) {
			if (a.isFunction(OnRemoteWebTwainNotFoundCallback)) {
				var Y = U._ssl ? U._portSSL : U._port;
				OnRemoteWebTwainNotFoundCallback(ae.name, U._ip, Y, U._ssl)
			}
			return
		}
		if (a.env.bMac) {
			if (af) {
				var W = a.one("#" + af);
				W.before(a.install._strNonInstallDIV);
				var ab = a.get(a.install._divDWTNonInstallContainerID);
				if (ab) {
					ab.style.width = X;
					ab.style.height = ac
				}
			...
			if (a.isFunction(OnWebTwainNotFoundOnMacCallback)) {
				var Z = ae.bChromeEdition ? ae.getChromeEditionPath() : ae.getPKGPath();
				OnWebTwainNotFoundOnMacCallback(ae.name, Z, ae.bChromeEdition, a.env.bIE, a.env.bSafari, a.detect.ssl, a.env.strIEVersion)
			}
		} else {
			if (a.env.bLinux) {
				if (af) {
				...
				if (a.isFunction(OnWebTwainNotFoundOnLinuxCallback)) {
					var aa, ad;
					aa = ae.getLinuxHTML5DebPath();
					ad = ae.getLinuxHTML5RpmPath();
					OnWebTwainNotFoundOnLinuxCallback(ae.name, aa, ad, ae.bChromeEdition, a.env.bIE, a.env.bSafari, a.detect.ssl, a.env.strIEVersion)
				}
			} else {
				if (V.match(/chrome\/([\d.]+)/) || V.match(/opera.([\d.]+)/) || V.match(/version\/([\d.]+).*safari/) || V.match(/firefox\/([\d.]+)/) || H.IfUseActiveXForIE10Plus == O || (!H.ActiveXInstallWithCAB && a.env.bIE)) {
					if (af) {
						var W = a.one("#" + af);
						W.before(a.install._strNonInstallDIV);
						var ab = a.get(a.install._divDWTNonInstallContainerID);
					...
					if (a.isFunction(OnWebTwainNotFoundOnWindowsCallback)) {
						var Z = "";
						if (!H.ActiveXInstallWithCAB) {
							if (ae.bChromeEdition) {
								Z = ae.getChromeEditionPath()
							} else {
								if (a.env.bIE) {
									if (a.env.bWin64) {
										Z = ae.getActiveXCABx64Path()
									} else {
										Z = ae.getActiveXCABx86Path()
									}
								} else {
									Z = ae.getMSIPath()
								}
							...
							Z = ae.bChromeEdition ? ae.getChromeEditionPath() : ae.getMSIPath()
						}
						OnWebTwainNotFoundOnWindowsCallback(ae.name, Z, ae.bChromeEdition, a.env.bIE, a.env.bSafari, a.detect.ssl, a.env.strIEVersion)
					}
				}
			}
		}
	...
	o = function(U) {
		var V = (navigator.userAgent.toLowerCase());
		if (V.match(/chrome\/([\d.]+)/)) {
			a.one("#" + U).before(a.install._strNonInstallDIV);
			a.show(a.install._divDWTNonInstallContainerID);
			a.hide(U);
			if (a.install._divDWTemessageContainer != "") {
				a.hide(a.install._divDWTemessageContainer)
	...
	a.detect.getVersionArray = function(U) {
		var V = U.toLowerCase().replace(a.product.name.toLowerCase() + " ", "");
		V = V.replace("trial".toLowerCase(), "");
		V = a.replaceAll(V, "[.]", ",");
		return V.split(",")
	};
	a.detect.isDWTVersionLatest = function(V) {
		if (a.isUndefined(V)) {
			return O
		}
		var Y = V.toString().toLowerCase();
		var X = a.detect.getVersionArray(Y),
			U, Z;
		if (a.env.bIE && (a.product.bPluginEdition || a.product.bActiveXEdition)) {
			...
			if (a.product.bPluginEdition) {
				...
				U = H.ServerVersionInfo
			}
		}
		U = a.replaceAll(new String(U), "[.]", ",");
		Z = U.split(",");
		index = X.length > Z.length ? X.length : Z.length;
	...
	var M = function(W, Z) {
			if (a.isFunction(OnWebTwainNeedUpgradeCallback)) {
				...
				if (a.env.bWin) {
					if (a.product.bChromeEdition) {
						U["default"] = a.product.getChromeEditionPath()
					} else {
						if (!H.ActiveXInstallWithCAB && a.env.bIE) {
							if (a.env.bWin64) {
								U["default"] = a.product.getActiveXCABx64Path()
							} else {
								U["default"] = a.product.getActiveXCABx86Path()
							}
						} else {
							U["default"] = a.product.getMSIPath()
						}
					}
					Y = EnumDWT_PlatformType.enumWindow
				} else {
					if (a.env.bLinux) {
						var X, V;
						X = a.product.getLinuxHTML5DebPath();
						V = a.product.getLinuxHTML5RpmPath();
						U["default"] = X;
						U.deb = X;
						U.rpm = V;
						Y = EnumDWT_PlatformType.enumLinux
					} else {
						if (a.product.bChromeEdition) {
							U["default"] = a.product.getChromeEditionPath()
						} else {
							U["default"] = a.product.getPKGPath()
						}
						Y = EnumDWT_PlatformType.enumMac
					}
				}
				OnWebTwainNeedUpgradeCallback(a.product.name, U, a.product.bChromeEdition, Y, a.env.bIE, a.env.bSafari, a.detect.ssl, a.env.strIEVersion, a.detect.bPromptJSOrServerOutdated, W, Z)
			}
		},
		m = function(U, W) {
			if (!a.detect.bNeedUpgradeEvent && !a.detect.isDWTVersionLatest(W.VersionInfo)) {
				a.detect.bNeedUpgradeEvent = B;
				if (!H.UseDefaultInstallUI && a.isFunction(H.OnWebTwainNeedUpgrade)) {
					H.OnWebTwainNeedUpgrade();
					return
				}
				if (W && W._customIP && (!a.isLocalIP(W._ip))) {
					if (a.isFunction(OnRemoteWebTwainNeedUpgradeCallback)) {
						var V = W._ssl ? W._portSSL : W._port;
						OnRemoteWebTwainNeedUpgradeCallback(a.product.name, W._ip, V, W._ssl)
					}
					return
				}
				a.one("#" + U).before(a.install._strNonInstallDIV);
	...
	var r = function(U) {
			var V = U || {};
			if (V.basePath) {
				a.env.basePath = V.basePath
			}
			if (V.pathType) {
				a.env.pathType = V.pathType
			}
			if (!a.isUndefined(V.bDiscardBlankImage)) {
				a.config.bDiscardBlankImage = V.bDiscardBlankImage
			}
			if (V.msgPrefix) {
				a.config.msgPrefix = V.msgPrefix
			}
			if (V.msgSuffix) {
				a.config.msgSuffix = V.msgSuffix
			}
			if (V.detectType) {
				a.detect.detectType = V.detectType
			}
			if (V.tryTimes) {
				a.detect.tryTimes = V.tryTimes
			}
			if (V.onReady) {
				H.OnWebTwainReady = V.onReady
			}
			if (V.onNotAllowedForChrome) {
				a.detect.onNotAllowedForChrome = V.onNotAllowedForChrome
			}
			if (V.onNoControl) {
		...
		G = function(X) {
			...
			if (U.length > 0) {
				...
				if (!a.isUndefined(X)) {
					ab = X._strDWTControlContainerID;
					Z = X
				} else {
					...
					if (a.product.bChromeEdition) {
						...
						if (Z.ErrorCode == -2207) {
							if (!H.UseDefaultInstallUI && a.isFunction(H.OnWebTwainNeedUpgradeWebJavascript)) {
								H.OnWebTwainNeedUpgradeWebJavascript(Z.ErrorString)
							} else {
		...
		h = function(U) {
			U.IfAllowLocalCache = B;
			U.BrokerProcessType = 1;
			U.IfShowFileDialog = B;
			U.IfDisableSourceAfterAcquire = B;
			if (a.env.bMac || a.env.bLinux) {
				U.ImageCaptureDriverType = 3
			}
		},
		u = function(U) {
			var V;
			if (!U || !U.ContainerId || U.ContainerId == "") {
				return O
			}
			if (!a.get(U.ContainerId)) {
				return O
			}
			return B
		},
		i = function(U) {
			if (u(U)) {
				var V;
				V = H.ContainerMap[U.ContainerId];
				if (V) {
					return O
				}
				return B
		...
		t = function() {
			if (!a.detect.StartWSByIPTimeoutId && a.detect.arySTwainsByIP.length == 0) {
				a.detect.StartWSByIPTimeoutId = setTimeout(function() {
					a.detect.StartWSByIPTimeoutId = O;
					a.startWSByIP()
				}, 100)
			}
		};
	a.detect.__WebTwainCommonMain = function(U, W) {
		var V = H.UseDefaultInstallUI;
		if (V) {
			U.onNotAllowedForChrome = o;
			U.onNoControl = k
		} else {
			U.onNotAllowedForChrome = H.OnWebTwainOldPluginNotAllowed;
			U.onNoControl = H.OnWebTwainNotFound
		}
		r(U);
		if (a.isFunction(W)) {
			W()
	...
	a.detect.__WebTwainMain = function(X, V) {
		var U, W;
		U = {
			containerID: X.ContainerId || "dwtcontrolContainer",
			width: X.Width,
			height: X.Height,
			HTTPPort: (a.detect.ssl ? 443 : 80)
		};
		a.detect.__WebTwainCommonMain(U, function() {
			var aa, Y, ab, Z;
			if (a.product.bChromeEdition) {
				U.onWSReady = function(ac) {
					a.replaceControl(U.containerID, ac);
					if (a.isFunction(V)) {
						V(ac)
					}
				};
				aa = a.DynamicWebTwain(U)
			} else {
				U.onPluginReady = function() {
					a.replaceControl(U.containerID, aa.getInstance())
				};
				U.onPluginNotReady = function() {
					if (a.isFunction(Dynamsoft_OnNotReady)) {
						Dynamsoft_OnNotReady()
					}
				};
				aa = new a.DynamicWebTwain(U);
				ab = 0;
				Z = H.Containers;
				...
				if (a.detect.arySTwains.length == ab) {
	...
	var I = function() {
		...
		e = function() {
			...
			for (V = 0; V < W.length; V++) {
				...
				if (u(U)) {
					if (U == X) {
						a.detect.__WebTwainMain(U, G);
						break
					}
					a.detect.__WebTwainMain(U)
				}
			}
		},
		y = function() {
		...
		z = function(ac, ab, Y, V) {
			var Z, W = ac,
				aa = false;
			if (!a.isString(W) || W == "" || !a.isFunction(a.UI.ImageUIView)) {
				aa = true;
				W = "_dwtNoUI_" + a.getRandom()
			}
			if (!a.product.bChromeEdition) {
				if (a.isFunction(V)) {
					V(R)
			...
			if (H.Containers && !aa) {
				var X, U = H.Containers;
				for (X = 0; X < U.length; X++) {
					if (U[X].ContainerId == W) {
						if (a.isFunction(V)) {
							V(C)
						}
						return O
			...
			a.html5.CreateDWTObject(W, ab, aa, function(ae) {
				DCP_DWT_OnClickCloseInstall();
				H.DynamicContainers.push(W);
				H.DynamicDWTMap[W] = ae;
				h(ae);
				var ad = ae.LogLevel;
	...
	H.CreateDWTObject = function(V, ab, W, aa, Z, U) {
		q();
	...
	var q = function() {
		...
		N = function(X, W) {
			var V = X.split(","),
				U = W.split(",");
			return (V.length > 3 && a.trim(V[0]) == U[0] && a.trim(V[1]) == U[1] && a.trim(V[2]) == U[2] && a.trim(V[3]) == U[3])
		},
		g = function(V, U) {
			if (a.isLocalIP(V)) {
				a.LS.item("DWT_port", U);
		...
		l = function(Y) {
			...
			return [V, X, U, a.replaceAll(H.ServerVersionInfo, ",", "."), ".zip", W].join("")
		},
		p = function(Y) {
			var U, V, X = "",
				W = Y;
			U = "DynamicWebTWAINModule";
			if (!K.bFileSystem) {
				X = "?t=" + a.getRandom()
			}
			V = H.Trial ? "Trial" : "";
		...
		s = function(Z, W, U) {
			var ad = O,
				...
				X = a.detect,
				Y = Z.dwtPort,
				ab = dynamsoft.navInfo,
				ac, V = 1;
			if (ab.bWin) {
				V = U > 0 ? (U + 1) : 1
			}
			if (X.dcpStatus == v) {
				if (a.isFunction(Z.callback)) {
					Z.callback()
				}
				return
			}
			if (Y) {
				...
				ac = [];
				a.each(X.ports, function(ae) {
					if (ab.bSSL) {
						ae.ssl && ac.push(ae.port)
					} else {
						!ae.ssl && ac.push(ae.port)
					}
				})
			}
			w.versionInfo(ac, function(af, ae) {
				if (ae && ae.length > 0) {
					if (N(ae, H.ServerVersionInfo)) {
						a.detect.hideMask();
						X.dcpStatus = v;
				...
				if (W) {
					if (V > 3) {
						a.detect.hideMask();
						X.dcpStatus = x;
						if (a.isFunction(Z.dwtFailedCallback)) {
							Z.dwtFailedCallback()
						}
						return
					} else {
					...
					var ah = l(H.ResourcesPath),
						ag = function(ak) {
							...
							dynamsoft.dcp.loadZip(ak, al, function() {
								...
								if (H.OnWebTwainInitMessage) {
									var am;
									if (ab.bSSL && a.env.bFirefox) {
										am = "Please REFRESH your browser.";
										H.OnWebTwainInitMessage && H.OnWebTwainInitMessage(am, EnumDWT_InitMsg.DownloadNotRestartError);
										return
									}
									am = "The Dynamic Web TWAIN module loaded.";
									H.OnWebTwainInitMessage(am, EnumDWT_InitMsg.Info)
								}
								setTimeout(function() {
									s(Z, B, V)
								}, 3000)
						...
						aj = function(ak, an, am) {
							...
							if (H.OnWebTwainInitMessage) {
								...
								H.OnWebTwainInitMessage(aq, EnumDWT_InitMsg.DownloadError)
							}
							if (a.isFunction(OnWebTWAINDllDownloadFailure)) {
								...
								ap = OnWebTWAINDllDownloadFailure(a.product.name, ao._errorCode, ao._errorString);
								if (al) {
									ap = O
								}
							}
					...
					if (H.OnWebTwainInitMessage) {
						...
						H.OnWebTwainInitMessage(ai, EnumDWT_InitMsg.Info)
					}
					new a.ajax({
						method: "GET",
						url: ah,
		...
		D = function(Z) {
			...
			if (X.length > 0 && X[0] === f) {
				var ad = function() {
						if (H.OnWebTwainInitMessage) {
							var af = "Please download and install the Dynamic Web TWAIN.";
							H.OnWebTwainInitMessage(af, EnumDWT_InitMsg.Info)
						}
						if (!a.isLocalIP(dynamsoft.dcp.ip)) {
							if (a.isFunction(OnRemoteWebTwainNotFoundCallback)) {
								var ae = K.bSSL;
								OnRemoteWebTwainNotFoundCallback(a.product.name, dynamsoft.dcp.ip, Z.dwtPort, ae)
							}
						} else {
							k()
						}
					...
					Y = function() {
						...
						if (a.isFunction(OnWebTwainShowUpdatingDialog)) {
							af = OnWebTwainShowUpdatingDialog()
						}
						if (!af) {
							var ae = ['<div class="dynamsoft-dwt-dlg-tips dynamsoft-dwt-dlg-center">', "Dynamsoft Service is updating ...", "</div>", '<div style="padding-top:35px"></div>'];
							H.ShowDialog(365, 0, ae.join(""), 0, 1)
						}
						return true
					},
					U = function() {
						...
						if (a.isFunction(OnWebTwainCloseUpdatingDialog)) {
							ae = OnWebTwainCloseUpdatingDialog()
						}!ae && H.CloseDialog();
						return true
					},
					aa = function(ae, af) {
					...
					ac = function() {
						if (!ab) {
							ac = null;
							Z.dwtFailedCallback = function() {
								if (a.isFunction(OnWebTWAINModuleDownloadManually)) {
									var ag, ah = {
										"default": p(H.ResourcesPath + "/MSI/")
									};
									if (K.bMac) {
										ag = EnumDWT_PlatformType.enumMac
									} else {
										if (K.bLinux) {
											ag = EnumDWT_PlatformType.enumLinux
										} else {
											ag = EnumDWT_PlatformType.enumWindow
										}
									}
									OnWebTWAINModuleDownloadManually(ah, ag, a.env.bSafari, a.detect.ssl)
								}
							};
							a.detect.showMask();
							s(Z, O, 0);
				...
				dynamsoft.dcp.checkDcpForUpdate(ad, V, W, Y, U, aa, ac)
			} else {
	...
	H.DeleteDWTObject = function(X) {
		var W = H.DynamicDWTMap,
			U;
		if (!a.product.bChromeEdition) {
			return O
		}
		...
		for (U = 0; U < H.DynamicContainers.length; U++) {
			if (H.DynamicContainers[U] == X) {
				H.DynamicContainers.splice(U, 1);
				break
			}
		}
		a.detect.hideMask();
	...
	H.GetWebTwain = function(V) {
		...
		W = H.DynamicContainers;
		if (!X && W && W.length > 0) {
			X = W[0]
		}
		W = null;
		if (X && a.get(X)) {
			var Y = H.ContainerMap;
			if (Y && (X in Y)) {
				return Y[X]
			}
			Y = H.DynamicDWTMap;
	...
	var S = function() {
		if (!a.isUndefined(H.ProductKey)) {
			setTimeout(function() {
				var U = a.env.bFileSystem,
					X = a.env.basePath,
					V = Dynamsoft.WebTwainEnv.ResourcesPath;
				if (!U) {
					var Z = V;
					if (typeof(Z) == "undefined" || Z == "") {
						V = X
					} else {
						if (Z.length > 0 && Z[0] != "/" && Z.indexOf("//") < 0 && X != "") {
							V = [X, "/", Z].join("")
						}
					}
					if (V.length > 0 && V[V.length - 1] == "/") {
					...
					Dynamsoft.WebTwainEnv.ResourcesPath = V
				}
				var Y = "?t=20180120",
					W = B;
				if (a.env.bSafari) {
				...
				a.getScript([V, "/dynamsoft.webtwain.install.js", Y].join(""), W, function() {
					var ab = [V, "/reference/dynamsoft_dwt_html5.css", Y].join(""),
						aa = function() {
	...
	a.main = function(W, U) {
		var V = function() {
				if (a.DynamicWebTwain) {
					...
					a.product.version = H.ServerVersionInfo;
		...
		H.UseDefaultInstallUI = O;
		H.OnWebTwainOldPluginNotAllowed = U.onNotAllowedForChrome;
		H.OnWebTwainNotFound = U.onNoControl;
		a.ready(function() {
			a.detect.__WebTwainCommonMain(U, X)
		})
	}
